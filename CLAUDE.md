...
- **חובות פתוחים ממעבר דגלי המפה לפאנל (wave 1, 08.09.2026):**
  ...
  3. **תקרית אמיתית (08.09.2026):** הרצת `scripts/seed-hybrid-slot-flags.ts` (כדי להוסיף את `enable_recommended_hybrid` כמפתח רביעי) דרסה בחזרה ל-`true` את `enable_route_stops`, אף שדוד כיבה אותו במפורש בפאנל אחרי גל 1. הסקריפט כתב `setDoc(..., PAYLOAD, {merge:true})` עם ארבעת המפתחות קשיחים ל-`true` **ללא קשר לערך הקיים במסמך** — "אידמפוטנטי" התייחס רק ל"אותה תוצאה בהרצה חוזרת", לא ל"בטוח מול החלטה אמיתית של מנהל בין ריצה לריצה". תוקן ידנית על ידי דוד מיד. **שיעור כללי לכל סקריפט seed/bootstrap עתידי על דגל Firestore הניתן לעריכה בפאנל: לקרוא את המסמך לפני הכתיבה ולמלא רק מפתחות שבאמת חסרים — לעולם לא להניח שהערך שהיה נכון בזמן הכתיבה הראשונה עדיין נכון.** (סטטוס תיקון: ראה החלטה בסשן/קומיט העדכני — מחיקת הסקריפט או חיזוקו.)
