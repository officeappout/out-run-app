...
- **אמת טריות לפני כל ניתוח.** לפני קריאת קוד/חוקים כבסיס למסקנה ("האם X מתוקן", אודיט, מיפוי) — `git fetch` ואז השוואת `HEAD` המקומי מול `origin/main`. עותק מיושן נראה **בדיוק** כמו ממצא אמיתי. קרה פעמיים תוך יומיים על אותו commit מיושן (`bdaa02ac`, 08.09.2026): פעם ראשונה — שלושה ממצאי ביצועים שגויים בסשן אחר; פעם שנייה (16.09.2026) — כמעט דיווח שווא על חשיפת נתוני בריאות חיה בפרודקשן, כש-SEC-02 שתוקן כבר ב-`origin/main` נראה "פתוח" מהעותק המיושן. הכלל היה מתועד קודם רק ב-SPEC-05 (מסמך של פרויקט אחד) — לכן לא נקרא ולכן חזר. כאן הוא נקרא בכל סשן.
- **worktree נפרד תמיד, גם לקריאה-בלבד בלי קוד.** "רק קוראים" אינו פוטר מהכלל למעלה — קריאה מעותק מיושן מייצרת בדיוק את אותה תמונה שגויה כמו כתיבה אליו.
- **תיעוד ותוצרי ביקורת נכנסים לגיט.** קובץ שקיים רק בתיקייה המקומית אינו נגיש מ-worktrees — סשן שעובד מ-worktree לא רואה אותו בכלל, ולא יודע שהוא לא רואה אותו. זה כבר גרם לטענה שגויה (`DECISION-INDEX.md` §10 טען "לא התחיל מימוש" על עבודה שפשוט לא הייתה נגישה לו) ולסיכון אובדן בפועל (14 מסמכי `docs/audit-2026-09/` ישבו על דיסק אחד בלי גיבוי, 17.09.2026). מסמך שנוצר/מתעדכן — `git add` ו-commit אותו, לא רק שומרים על הדיסק.
...
- **חובות פתוחים ממעבר דגלי המפה לפאנל (wave 1, 08.09.2026):**
  ...
  3. **תקרית סגורה (08.09.2026):** הרצת `scripts/seed-hybrid-slot-flags.ts` (כדי להוסיף את `enable_recommended_hybrid` כמפתח רביעי) דרסה בחזרה ל-`true` את `enable_route_stops`, אף שדוד כיבה אותו במפורש בפאנל אחרי גל 1 — הסקריפט כתב `true` לארבעת המפתחות ללא תלות בערך הקיים במסמך; "אידמפוטנטי" התייחס רק ל"אותה תוצאה בהרצה חוזרת", לא ל"בטוח מול החלטה אמיתית של מנהל בין ריצה לריצה". תוקן ידנית על ידי דוד מיד. **פתרון:** הסקריפט נמחק לגמרי (מיצה את תפקידו החד-פעמי, אין לו יותר שימוש לגיטימי) — הנימוק המלא + השיעור הכללי לכל סקריפט seed/bootstrap עתידי (לקרוא את המסמך לפני הכתיבה, למלא רק מפתחות שבאמת חסרים) מתועדים כעת ב-`src/hooks/feature-flag-defs.ts` עצמו, לא תלויים בקובץ שנמחק.
