...
import {
  WayInfo, buildWayGrid, segsFromWays, computeRouteComposition, pathLenM,
  RECREATIONAL_DEDICATED_HIGHWAY, RECREATIONAL_ORDINARY_HIGHWAY, WayCategory,
} from './lib/route-composition-classify';
  ...
  for (let a = 0; a < 6; a++) for (const m of MIRRORS) {
    try {
      const buf: Buffer = await new Promise((res, rej) => {
        const req = https.request(m, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'OUT/1.0 (office@appout.co.il) quality-cert-stage1' } }, r => {
...
const BBOX_MARGIN_DEG = 0.005; // ~550m
...
async function main() {
  ...
  for (const [city, cityRoutes] of Array.from(byCity.entries())) {
    ...
    for (const r of validRoutes) for (const [lat, lng] of r.path) {
    ...
    console.log(`  bbox (derived from ${validRoutes.length} route paths + margin): ${bb}`);

    console.log(`  Fetching highway ways …`);
    const wayData = await overpass(`[out:json][timeout:180];way["highway"~"^(${HIGHWAY_VOCAB.join('|')})$"](${bb});out geom tags;`);
    ...
    console.log(`  ${allWays.length} ways fetched (${roadWays.length} road-category for sidewalk-parallel check).`);
    ...
    const lampData = await overpass(`[out:json][timeout:180];node["highway"="street_lamp"](${bb});out;`);
    ...
    for (const r of validRoutes) {
      const comp = computeRouteComposition(r.path, waysById, allGrid, roadGrid, wayCategoryCache);
