...
## 15. §5 (מחולל QR) + §4 (אנליטיקה) — מומשו (06.09.2026)

David אישר בדיקת שטח בפרודקשן (Smart Link עובד קצה-לקצה) ואישר לפתוח את §5+§4, בצמצום ליעד ≤16 שעות משולב.
...
### 15.2 §4 — אנליטיקה (per-link, `/admin/links/[id]`)

**התשובה לשאלת ה-TTL**: כן, זול, ובנוי מההתחלה כפי שביקשת. `marketing_links/{id}/daily_stats/{YYYY-MM-DD}` — מסמך רולאפ **קבוע, בלי TTL**, נכתב ב-batch אחד יחד עם רשומת הקליק (`link-click-handler.ts`) — עלות: כתיבת Firestore נוספת אחת לקליק, לא יותר. כל הפילוחים המצטברים (מכשיר/שעה/יום-בשבוע/מדינה/עיר/מגמה יומית) קוראים מ**הרולאפ הקבוע**, לא מרשומות הקליק — **בלי הגבלת 30 יום**. רק ייצוא ה-CSV (פר-קליק, לא מצטבר) עדיין מוגבל ל-30 יום כי הוא היחיד שצריך את הרשומה הגולמית.

- **שעון**: כל bucketing של שעה-ביום/יום-בשבוע הוא ב-**שעון ישראל** (`Asia/Jerusalem`, DST-aware דרך `Intl`, לא UTC) — `src/lib/israelTime.ts`, טהור, בלי תלות ב-Firebase, משותף לכתיבה (`link-stats-write.ts`, Admin SDK) ולקריאה (`link-stats.ts`, client SDK) כדי ששתי השכבות לא יסתרו זו את זו. **חשוב**: זו הסיבה שיש קובץ טהור נפרד — בלי זה, "רוב הסריקות בערב" היה יכול להיות שקר לפי 2-3 שעות.
- **`city` נוסף** לרשומת הקליק (`x-vercel-ip-city`, מפוענח מ-URL-encoding) — לא היה שם קודם.
- **מסך** `/admin/links/[id]/page.tsx` — טבלאות + "היסטוגרמה" פשוטה (bar CSS, בלי ספריית גרפים) לשעה/יום, טבלאות למכשיר/מדינה/עיר/מגמה, בורר טווח תאריכים, ייצוא CSV, הערת TTL גלויה.
- **firestore.rules**: בלוק חדש ל-`daily_stats` — קריאה לכל אדמין (כמו ה-doc האב), כתיבה רק Admin SDK.
- 27 טסטים חדשים (bucketing, בניית ה-increment, אגרגציה, CSV) — כולם טהורים, בלי Firestore אמיתי.

## 16. שדרוג עיצוב QR — ספרייה חדשה + סגנון + מד סיכון + מחשבון הדפסה (06.09.2026)

David דיווח שה-QR "עובד אבל נראה גנרי" וביקש: סגנון נקודות/פינות, סליידר גודל לוגו, ריפוד לבן, גרדיאנט, מד סיכון סריקה, מחשבון גודל הדפסה — ותיקון חוזר לבאג כפתור-ההורדה (התיקון המקורי אבד בתקרית קונפליקט git תלת-שלבית עם סשן מקביל, שהובילה גם לכלל ה-worktree-per-session החדש ב-`CLAUDE.md`).

### 16.1 החלטת ספרייה — מעבר מ-`qrcode` ל-`qr-code-styling`

`qrcode` (§15) מייצר רק מודולים ריבועיים — אין תמיכה בסגנונות נקודות/פינות/גרדיאנט. שתי אופציות נשקלו: (א) מעבר ל-`qr-code-styling` (canvas/DOM-based, מוותר על חלק מהטהרות), (ב) הישארות עם `qrcode` וציור ידני של הצורות כ-SVG (יותר עבודה, שומר טהרות מלאה). **הוחלט על (א)** — האפקטיביביליטי (Effort) לא היה משמעותית שונה (ציור ידני של rounded/dots/classy הוא גיאומטריה לא-טריוויאלית), וה-Verification הבאה הורידה את הסיכון המרכזי:

- `qr-code-styling@1.9.2`, MIT, תלות יחידה (`qrcode-generator`), 648KB, עודכן לאחרונה 04/2025.
- **נבדק ישירות**: `grep -c "require(" lib/qr-code-styling.js` → **0** — אין תלות Node-only מוסתרת בבאנדל הקומפילד. `canvas`/`jsdom` מוזכרים רק כ-type-level, מוזרקים ידנית ע"י הקורא ל-server-side rendering — לא רלוונטי לקומפוננטת `'use client'` שרצה רק בדפדפן. זה מוריד ישירות את הסיכון לחזור על באג ה-production build שנפל בסבב הקודם (Admin SDK שנדלף ל-client bundle).

**מה אבד / מה מפצה**: הטסטים על מניפולציית מחרוזת ה-SVG הידנית (`generateQrSvgString`, הזרקת `<image>`/`<rect>`) הוסרו — הקוד עצמו הוחלף ברינדור הטבעי של הספרייה. מפצה: (1) בניית ה-`Options` object לספרייה הופרדה לפונקציה טהורה נפרדת — `buildQrCodeStylingOptions()` ב-`qr-generator.ts` — שכן היא בונה אובייקט בלבד, בלי רינדור, ונשארת נבדקת ביחידה; (2) שני החישובים הטהורים החדשים (מד סיכון, מחשבון הדפסה) מקבלים כיסוי טסטים מלא — ערך גבוה יותר מבדיקת concatenation של מחרוזת; (3) רינדור ה-PNG כבר לא היה נבדק (browser-only) גם קודם — לא פער חדש. סה"כ: 31 טסטים ב-`qr-generator.test.ts` (היו 12).

### 16.2 בקרות עיצוב חדשות (`QrCodeGenerator.tsx`)

- **סגנון נקודות**: ריבועי (ברירת מחדל) / מעוגל / נקודות / Classy — `dotsOptions.type`.
- **סגנון פינות** (נפרד מהנקודות): מרובע / מעוגל / נקודה — `cornersSquareOptions.type`.
- **גודל לוגו**: סליידר 12%–22%, ברירת מחדל 18%. הקיפאון ב-22% נאכף בשתי שכבות: טווח ה-`<input type="range">` עצמו וגם `clampLogoSizeRatio()` בתוך `buildQrCodeStylingOptions` (הגנה גם אם קורא עתידי ישכח לקצץ קודם).
- **ריפוד לבן מסביב ללוגו**: toggle, ממופה ל-`imageOptions.margin` (פיקסלים, יחסי לגודל הרינדור).
- **גרדיאנט**: checkbox + בורר צבע שני; כשמופעל — `dotsOptions.gradient` לינארי (45°) מ-`dark` אל הצבע השני. **בדיקת ניגודיות מול הגרדיאנט**: `computeEffectiveContrastRatio()` בודקת את *שני* קצוות הגרדיאנט מול הרקע ומחזירה את הגרוע מביניהם — כדי שגרדיאנט לא "יסתיר" זוג-צבעים בעייתי מאחורי הקצה הטוב יותר שלו.

### 16.3 מד סיכון סריקה (🟢/🟡/🔴) — היוריסטיקה, לא בדיקה מדעית

`computeScanRiskLevel()` ב-`qr-generator.ts` — worst-of-three: לוקח את הגרוע מבין שלושה גורמים (לא ממוצע), עם ספים מתועדים בקוד:

| גורם | 🟢 נמוך | 🟡 בינוני | 🔴 גבוה |
|---|---|---|---|
| גודל לוגו | ≤18% (ברירת המחדל) | 18–20% | מעל 20% (עד התקרה 22%) |
| סגנון נקודות | ריבועי | מעוגל / נקודות | Classy |
| ניגודיות | ≥7:1 | 4–7:1 | מתחת ל-4:1 (עד הרצפה הקשיחה 3:1) |

זו לא בדיקה מדעית — מטרתה למנוע טעות דפוס יקרה, לא להחליף סריקת בדיקה בפועל (שנשארת חובה, ראו האזהרה הקיימת).

### 16.4 מחשבון גודל הדפסה

`computeMinPrintWidthCm(scanDistanceMeters)` — כלל אצבע "מרחק סריקה ≈ פי 10 מרוחב ה-QR": `width_cm = distance_m × 10`. שדה קלט "מרחק סריקה צפוי (מ׳)" ליד התצוגה המקדימה, ברירת מחדל 1 מ׳ (→ 10 ס״מ).

### 16.5 תיקון כפתור ההורדה (חוזר)

הלייבל של כל color-input הועבר להיות **מעל** ה-swatch (במקום לצידו) + נוסף `border-t` + כותרת "הורדה" לפני שורת כפתורי ההורדה — כדי שה-swatch הכהה (מלבן צבוע בלי טקסט, בעיצוב דפדפן) לא ייקרא כאילו הוא כפתור-הורדה שלישי בלי טקסט.

### 16.6 מה נבדק

`npx vitest run` (31/31 בקובץ הזה, ה-4 כשלים האחרים בסוויט הכללית לא קשורים — streak/window-start, כנראה תלויי-תאריך, לא נגעתי), `npx tsc --noEmit` (אין שגיאות חדשות בקבצים ששונו — 795 שורות שגיאה קיימות מראש ב-`scripts/`/`functions/`/עמודים אחרים, לא קשורות), `npx eslint` (נקי), **`npx next build` מלא — success, exit 0** — האימות שהיה חסר בסבב שהפיל את הפרודקשן.

### 16.7 תקרית פרודקשן (06.09.2026) — קריסת `QRCodeStyling` + הלקח הכללי

מיד אחרי המיזוג, פתיחת דרואר העריכה הפילה את כל העמוד ל-error boundary: `TypeError: Cannot read properties of undefined (reading 'hideBackgroundDots')` בתוך הבנאי של `QRCodeStyling`.

**שורש הבעיה, מאומת מול הספרייה האמיתית (לא הנחה)**: `buildQrCodeStylingOptions` קבע `imageOptions: undefined` בכל פעם שאין עדיין לוגו — וזה בדיוק המצב ברגע שהדרואר נפתח, כי הלוגו נטען א-סינכרונית. `qr-code-styling` ממזג את האפשרויות שהמפתח מעביר מעל ברירות המחדל שלה עם `Object.assign` שטוח — מפתח **קיים** עם ערך `undefined` **דורס** את אובייקט ברירת המחדל של הספרייה (בעוד מפתח **חסר** לא היה דורס כלום), והבנאי שלה קורא אז ל-`imageOptions.hideBackgroundDots` בלי guard — קורס. אומת אמפירית מול הספרייה המותקנת בפועל (לא רק קריאת קוד): אותה שגיאה בדיוק שיחזרה בסקריפט Node עצמאי.

**תיקון**: conditional spread (`...(condition ? {imageOptions: {...}} : {})`) במקום ternary-ל-undefined — כך שהמפתח **נעדר לגמרי** כשאין לוגו, לא נוכח עם ערך undefined.

**למה 31 הטסטים לא תפסו את זה**: הבדיקה הישנה `expect(options.imageOptions).toBeUndefined()` עוברת זהה בין "מפתח נעדר" ל"מפתח נוכח עם ערך undefined" — היא לא יכולה להבחין בין השניים. הוחלפה בבדיקת `hasOwnProperty`. נוספו גם 3 טסטים שבונים `new QRCodeStyling(...)` עם הספרייה האמיתית (לא מוק) בדיוק בצורה שהפרודקשן שולח — אומתו ש**נכשלים** עם הודעת השגיאה המדויקת של הפרודקשן מול הקוד הישן, לפני שאומתו שעוברים מול התיקון.

**לקח כללי (מתועד גם ב-`axioms.md` §24, כי זה יחזור עם ספריות אחרות)**: בכל בניית options object לספרייה חיצונית שממזגת מעל ברירות מחדל משלה — מפתח נוכח עם `undefined` ≠ מפתח נעדר. שווה לבדוק את הדפוס הזה בכל אינטגרציה עתידית עם ספרייה חדשה, לא רק כאן.

**אימות אחרי הפריסה**: `git rev-list --left-right --count origin/main...HEAD` ו-`git merge-base --is-ancestor` הופעלו מול `origin/main` (לא מול `main` המקומי, שעלול להיות מיושן/מוזז — בדיוק הסיבה שבדיקת הכלי בפעם הקודמת נתנה תשובה שגויה). David אימת ידנית בפרודקשן: הדרואר נפתח, המחולל עובד, אין שגיאות.
