...
> ## ⚠️ תיקון עובדתי (05.09.2026, לאחר כתיבת המסמך) — `onelink.to` ≠ AppsFlyer OneLink
>
> **הפרומפט המקורי ואני שנינו טעינו.** `onelink.to` (שאליו מצביעים `nmpcb5` ועוד כ-20 קישורים) הוא שירות **עצמאי** של חברה בשם OLTO — מחולל smart-link פשוט להפניה לחנויות + QR, **בלי SDK ובלי אטריביושן**. AppsFlyer's OneLink יושב תחת `onelink.me`, מוצר אחר לגמרי, ואין לנו איתו שום קשר (לא SDK, לא חשבון, לא נתונים).
>
> **מה זה משנה במסמך הזה:**
> - **§3 (המחקר על AppsFlyer — Pull/Push API, SKAN/ATT, תוכנית Zero ושינוי המחירים באוגוסט 2026)** — כל הממצאים שם עדיין **עובדתית נכונים על AppsFlyer עצמו**, אבל הם **אף פעם לא תיארו את המצב שלנו בפועל** — הם היו תמיד דיון היפותטי "אם נאמץ AppsFlyer בעתיד" (ראה גם §11.2/11.3 ו-§12.4 ב-`docs/architecture/marketing-attribution.md`), לא מגבלה שחלה עלינו היום. **מחקנו את זה כמגבלה נוכחית** — זו לא הייתה מעולם.
> - **שאלה פתוחה §6.2 ("תוכנית AppsFlyer הנוכחית של OUT") — מבוטלת.** אין לנו תוכנית AppsFlyer בכלל, השאלה לא רלוונטית.
> - **שאלה פתוחה §6.6 (דיוק מחיר AppsFlyer Growth)** — נשארת רלוונטית **רק** אם/כאשר נשקול לאמץ AppsFlyer בעתיד ל-iOS (§11.2), לא כעלות נוכחית.
> - **H1/H2/§4/§6.1 (הברקוד מקודד ל-onelink.to הגולמי או לקישור שלנו)** — נשארים נכונים **כפי שהם**: `onelink.to` (ה-שירות האמיתי) עדיין לא מחובר לשום קוד שלנו, ועדיין לא מודד install/user — המסקנה התפעולית (לוודא שה-QR הפיזי מצביע ל-`/api/links/{id}/click` שלנו) לא משתנה, רק ה"למה" משתנה: זה לא כי OneLink-של-AppsFlyer "עוקף" אותנו, אלא כי onelink.to פשוט לא מדווח לנו כלום מלכתחילה — אין לו על מה לדווח.
>
> ראה `docs/architecture/marketing-attribution.md` §12 לניתוח מתוקן מלא של onelink.to האמיתי (כולל מגבלות תוכנית אמיתיות — 3/10/200/500 קישורים, retention 90 יום ב-Free מול שנתיים בתשלום).
