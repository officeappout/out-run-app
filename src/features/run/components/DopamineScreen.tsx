...
export default function DopamineScreen() {
  const { totalDistance } = useRunStore();
  ...
  const bonusSteps = [
    { id: 'dist', label: 'בונוס מרחק!', amount: Math.floor(totalDistance * 10), icon: '🏃‍♂️', delay: 0.5 },
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-slate-50 flex flex-col items-center justify-center overflow-hidden">
      {/* אנימציית המטבעות הנופלים */}
      ...
      <motion.div className="relative z-10 flex flex-col items-center">
        ...
        {/* המטבע המרכזי בעיצוב "כללית" */}
        <motion.div 
          animate={{ 
            scale: activeBonus ? [1, 1.15, 1] : 1,
            rotate: activeBonus ? [0, 2, -2, 0] : 0
          }}
          className="w-72 h-72 bg-white rounded-full shadow-[0_20px_60px_rgba(0,178,255,0.25)] border-[10px] border-white flex flex-col items-center justify-center relative"
        >
          {/* Progress Circle */}
          ...
          <div className="flex flex-col items-center z-10">
            <motion.span 
              ...
              initial={{ scale: 0.7 }}
              animate={{ scale: 1 }}
              className="text-8xl font-[1000] text-slate-800 tracking-tighter leading-none"
            >
              {totalCoins}
            </motion.span>
            <div className="flex items-center gap-2 mt-2">
               <span className="w-6 h-6 bg-yellow-400 rounded-full border-2 border-white shadow-sm" />
               <span className="text-2xl font-black text-blue-500 uppercase tracking-widest">
                 עמירם
               </span>
            </div>
          </div>
        ...
        >
          <h2 className="text-4xl font-black text-slate-800 tracking-tight">אליפות! 🏆</h2>
          <p className="text-slate-400 font-bold mt-2 text-lg">עמדת ביעד הצעדים השבועי!</p>
        </motion.div>
      </motion.div>
    </div>
  );
