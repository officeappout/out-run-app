...
import {
  Plus, Trash2, Save, Loader2, Building2, Tag, ArrowRight,
  Search, ChevronDown, Dumbbell, ImageIcon, ImagePlus, X as XIcon, Lock, AlertTriangle,
} from 'lucide-react';
import { safeRenderText } from '@/utils/render-helpers';

// ── Circle polygon generator for authority safe-zone ──────────────────
...
const DEFAULT_AUTHORITY_RADIUS_KM = 5;
...
export interface ParkFormProps {
  /** Existing park data for edit mode; omit for create mode */
  initialData?: Park | null;
  /** Lock the authority to this ID (hides authority picker) */
  lockedAuthorityId?: string;
  /** Display name for the locked authority */
  lockedAuthorityName?: string;
  /** Pre-select an authority but still allow changes (super admin) */
  defaultAuthorityId?: string;
  /** 'published' = super admin default; 'pending' = authority admin default */
...
interface ParkFormData extends Omit<Park, 'id' | 'facilities' | 'gymEquipment'> {
  gymEquipment: (ParkGymEquipment & { equipmentName?: string })[];
...
export default function ParkForm({
  ...
  const [loadingAuthorities, setLoadingAuthorities] = useState(!lockedAuthorityId);
  const [outdoorBrands, setOutdoorBrands] = useState<OutdoorBrand[]>([]);
  const [loadingBrands, setLoadingBrands] = useState(true);

  const [authoritySearch, setAuthoritySearch] = useState(lockedAuthorityName || '');
  ...
  const [boundaryGeoJSON, setBoundaryGeoJSON] = useState<GeoJSON.Feature<GeoJSON.Polygon> | null>(null);
  const [isOutOfBounds, setIsOutOfBounds] = useState(false);
  const isStrictBoundary = !!lockedAuthorityId;

  const effectiveAuthorityId = lockedAuthorityId || defaultAuthorityId || initialData?.authorityId;

  const { register, control, handleSubmit, watch, setValue, formState: { errors } } = useForm<ParkFormData & { authorityId?: string }>({
    defaultValues: {
      name: initialData?.name || '',
      description: initialData?.description || '',
      location: initialData?.location || { lat: 32.0853, lng: 34.7818 },
      facilityType: 'gym_park',
      sportTypes: initialData?.sportTypes || [],
      featureTags: initialData?.featureTags || [],
      gymEquipment: initialData?.gymEquipment?.map(eq => ({
  ...
  useEffect(() => {
    ...
    if (selectedAuthorityIdValue) {
      ...
      if (found) setAuthoritySearch(found.name);
    }
  }, [selectedAuthorityIdValue, authorities, lockedAuthorityId]);

  // ── Load supporting data ────────────────────────────────────────────
  useEffect(() => {
    loadGymEquipment();
    if (!lockedAuthorityId) loadAuthorities();
    loadOutdoorBrands();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => {
    if (initialData?.gymEquipment && gymEquipmentList.length > 0) {
      const terms: Record<number, string> = {};
      const bTerms: Record<number, string> = {};
      initialData.gymEquipment.forEach((eq, idx) => {
        const found = gymEquipmentList.find(e => e.id === eq.equipmentId);
        terms[idx] = found?.name || '';
        bTerms[idx] = eq.brandName || '';
      });
      setEquipmentSearchTerms(prev => ({ ...prev, ...terms }));
      setBrandSearchTerms(prev => ({ ...prev, ...bTerms }));
    }
  }, [initialData, gymEquipmentList]);

  useEffect(() => {
    if (lockedAuthorityId || !initialData?.authorityId) return;
    if (authorities.length > 0) {
      const found = authorities.find(a => a.id === initialData.authorityId);
      if (found) setAuthoritySearch(found.name);
    } else {
      getAuthority(initialData.authorityId)
        .then(a => { if (a) setAuthoritySearch(a.name); })
        .catch(() => {});
    }
  }, [initialData, authorities, lockedAuthorityId]);

  // ── Load boundary for authority safe-zone ───────────────────────────
  useEffect(() => {
    const resolveId = lockedAuthorityId || selectedAuthorityIdValue;
    if (!resolveId) { setBoundaryGeoJSON(null); return; }
  ...
  // ── Submit ──────────────────────────────────────────────────────────
  const isSaveBlocked = isStrictBoundary && isOutOfBounds;
  ...
  const onSubmit = async (data: ParkFormData) => {
    ...
    if (isSaveBlocked) {
      alert('לא ניתן לשמור — המיקום מחוץ לגבולות הרשות. יש להזיז את הסימון לתוך השטח המסומן במפה.');
      return;
    }

    try {
      setIsSubmitting(true);
      setUploadProgress(isEditMode ? 'מתחיל עדכון...' : 'מתחיל העלאה...');
      ...
      const parkImageUrl = parkImages[0] || initialData?.image || '';

      const processedGymEquipment: ParkGymEquipment[] = data.gymEquipment.map(eq => ({
        equipmentId: eq.equipmentId,
        brandName: eq.brandName,
      }));

      const selectedAuthorityId = lockedAuthorityId || data.authorityId;
      ...
      if (isPending) {
        parkData.contentStatus = 'pending_review';
        parkData.published = false;
        parkData.origin = 'authority_admin';
      } else if (!isEditMode) {
        parkData.origin = 'super_admin';
      }

      setUploadProgress('שומר נתונים...');
      ...
      if (isEditMode && initialData) {
        await updatePark(initialData.id, parkData, adminInfo);
      } else {
        parkData.createdAt = serverTimestamp();
        parkData.updatedAt = serverTimestamp();
        parkData.amenities = null;
        await createPark(parkData as any, adminInfo);
      }

      setUploadProgress('הצלחה!');
      const defaultRedirect = isEditMode ? '/admin/locations' : '/admin/parks';
      setTimeout(() => router.push(redirectPath || defaultRedirect), 1000);
    } catch (error: unknown) {
      const msg = error instanceof Error ? error.message : 'Unknown error';
  ...
  // ── Render ──────────────────────────────────────────────────────────
  const formTitle = isEditMode ? 'עריכת פארק' : 'הוספת פארק וגינה';
  const submitLabel = isEditMode ? 'שמור שינויים' : 'שמור פארק';
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-28" dir="rtl">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black text-gray-900">{formTitle}</h1>
          {isEditMode && initialData && (
            <p className="text-sm text-gray-500 mt-1">{initialData.name}</p>
          )}
          {isPending && (
            <p className="text-xs text-amber-600 font-bold mt-1">
              המיקום יישלח לאישור מנהל העל לפני פרסום
            </p>
          )}
        </div>
        <button
        ...
        >
          ...
          <span>{isSubmitting ? 'שומר...' : submitLabel}</span>
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-6">
          ...
          {/* ═══ Authority — locked badge or searchable select ═══ */}
          {lockedAuthorityId ? (
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 p-3 bg-blue-50 rounded-xl border-2 border-blue-200">
                ...
                <span className="font-bold text-blue-800">{lockedAuthorityName || lockedAuthorityId}</span>
              </div>
            </div>
          ) : (
                      ...
                      filteredAuthorities.map((authority) => (
                        ...
                        >
                          ...
                          <span className="text-xs text-gray-400 mr-2">
                            {authority.type === 'regional_council'
                              ? '(מועצה אזורית)'
                              : authority.type === 'city'
                                ? '(עירייה)'
                                : '(מועצה מקומית)'}
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-6">
          ...
          <p className="text-sm text-gray-500">
            {isEditMode ? 'לחץ על המפה כדי לעדכן את מיקום הפארק' : 'לחץ על המפה כדי לסמן את מיקום הפארק'}
          </p>
          ...
          <Controller
            ...
            render={({ field: { value, onChange } }) => (
              <LocationPicker
                ...
                enforceStrict={isStrictBoundary}
              />
            )}
          />
          ...
          {isOutOfBounds && isStrictBoundary && (
            <div className="flex items-center gap-2 p-3 bg-red-50 rounded-xl border border-red-200 text-red-700 text-sm font-bold" dir="rtl">
              <AlertTriangle size={16} />
              לא ניתן לשמור — המיקום מחוץ לגבולות הרשות
            </div>
          )}
        </div>
      </form>
        ...
        <div className="max-w-4xl mx-auto px-4 pb-4 pointer-events-auto">
          <div className="bg-white/80 backdrop-blur-xl border border-gray-200/60 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] rounded-2xl p-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 text-sm text-gray-500">
              <div className={`w-2 h-2 rounded-full ${isEditMode ? 'bg-amber-400' : 'bg-green-400'} animate-pulse`} />
              <span className="hidden sm:inline">
                {isEditMode && initialData
                  ? `עריכת פארק — ${initialData.name}`
                  : 'טופס הוספת פארק'}
              </span>
            </div>
            <button
              ...
              onClick={handleSubmit(onSubmit)}
              disabled={isSubmitting || isSaveBlocked}
              className={`flex items-center gap-2 text-white px-8 py-3 rounded-xl font-bold text-base shadow-lg transition-all disabled:opacity-50 ${
                isSaveBlocked ? 'bg-red-500' : 'bg-green-600 hover:bg-green-700 hover:shadow-xl'
              }`}
            >
              {isSubmitting ? <Loader2 className="animate-spin" size={20} /> : isSaveBlocked ? <AlertTriangle size={20} /> : <Save size={20} />}
              <span>{isSubmitting ? 'שומר...' : isSaveBlocked ? 'מיקום מחוץ לגבולות' : submitLabel}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <MediaLibraryModal
        ...
        authorityId={lockedAuthorityId || initialData?.authorityId}
        scope="locations"
      />
    </div>
  );
