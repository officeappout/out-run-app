...
export default function ApprovalPreviewMap({ geometry }: ApprovalPreviewMapProps) {
  // Initial center: the point / climb center / first route waypoint.
  const initial =
    geometry.kind === 'point'
      ...
      : geometry.kind === 'climb'
        ...
        : geometry.path.length > 0
          ? { longitude: geometry.path[0][0], latitude: geometry.path[0][1], zoom: 14 }
          : { longitude: 34.7818, latitude: 32.0853, zoom: 12 };
  ...
  const handleLoad = useCallback(
    (e: any) => {
      ...
      try {
        if (geometry.kind === 'route' && geometry.path.length >= 2) {
          let minLng = Infinity, minLat = Infinity, maxLng = -Infinity, maxLat = -Infinity;
          for (const [lng, lat] of geometry.path) {
  ...
  const routeLine =
    geometry.kind === 'route' && geometry.path.length >= 2
      ? { type: 'Feature' as const, properties: {}, geometry: { type: 'LineString' as const, coordinates: geometry.path } }
      : null;
  ...
  return (
    ...
    >
      ...
      {geometry.kind === 'route' && geometry.path.length >= 2 && (
        <>
          <Marker longitude={geometry.path[0][0]} latitude={geometry.path[0][1]} anchor="center">
            <div className="w-7 h-7 rounded-full bg-emerald-500 border-2 border-white shadow-lg" />
          </Marker>
          <Marker
            longitude={geometry.path[geometry.path.length - 1][0]}
            latitude={geometry.path[geometry.path.length - 1][1]}
            anchor="center"
          >
