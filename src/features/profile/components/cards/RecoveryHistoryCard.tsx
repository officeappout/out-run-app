...
import { Moon, Coins, Clock } from 'lucide-react';
import { WorkoutHistoryEntry } from '@/features/workout-engine/core/services/storage.service';
import { IS_COIN_SYSTEM_ENABLED } from '@/config/feature-flags';

interface RecoveryHistoryCardProps {
  workout: WorkoutHistoryEntry;
  onClick: () => void;
}

// Mirrors StrengthHistoryCard.tsx's shape/layout closely (closest match —
// no GPS/pace/distance, duration + coins stats). Slate accent is a
// deliberately distinct, subtle hue from Strength's purple and Activity's
// blue — echoes the muted grey-blue already used for "recovery" elsewhere
// in the app's color system (running-zone recovery, .claude/knowledge/
// color-system.md §2.1).
export default function RecoveryHistoryCard({ workout, onClick }: RecoveryHistoryCardProps) {
  ...
  return (
    ...
    >
      <div className="flex flex-row gap-4 p-4">
        {/* Left: Recovery Icon */}
        <div className="w-24 h-24 rounded-lg overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200 flex-shrink-0 flex items-center justify-center">
        ...
        <div className="flex-1 flex flex-col justify-between min-w-0">
          ...
          <div>
            ...
            <p className="text-sm font-semibold text-gray-700">אימון התאוששות</p>
          </div>
