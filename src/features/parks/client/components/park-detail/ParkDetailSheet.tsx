...
export default function ParkDetailSheet({ isOpen, onClose, onStartWorkout, userLocation }: ParkDetailSheetProps) {
  ...
  // render of a park that DID point-fetch successfully.
  useEffect(() => {
    if (!isOpen || !park || fetchedPark) return;
    const missing: string[] = [];
    if (park.description === undefined) missing.push('description');
    if (park.featureTags === undefined) missing.push('featureTags');
    if (park.city === undefined) missing.push('city');
    if (park.status === undefined) missing.push('status');
    if (park.facilityType === 'gym_park' && park.gymEquipment === undefined) missing.push('gymEquipment');
    if (missing.length > 0) {
      console.warn(
        `[ParkDetailSheet] TRIPWIRE: fallback render with incomplete park data — missing [${missing.join(', ')}], park ${park.id}, point-fetch not yet resolved or failed`,
      );
    }
