...
import { Route, ActivityType } from '@/features/map/types/map-objects.type'; // שים לב לנתיב הייבוא, השתמשתי ב-@ לנוחות
        ...
        {mode === 'discover' && routes.length > 0 ? (
          ...
          >
            {routes.map((route, index) => {
              ...
              // 👇 תיקון בטיחות: שימוש ב-0 כברירת מחדל אם הזמן חסר
          ...
          <div className="px-4">
            <div className="max-w-md mx-auto bg-white rounded-3xl p-6 shadow-2xl text-center border border-gray-100">
              ...
              <button
                onClick={() => { if (onStartWorkout) onStartWorkout(); }}
