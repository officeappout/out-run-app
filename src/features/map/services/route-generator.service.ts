...
export async function generateDynamicRoutes(
  ...
  for (let i = 0; i < routeCombinations.length; i++) {
    ...
    try {
      ...
      const distanceDiff = Math.abs(routeDistanceKm - targetDistance);

      // Flexible distance check
      if (distanceDiff > targetDistance * 0.6) {
        console.warn(`[RouteGenerator] Route ${i} REJECTED: distance ${routeDistanceKm.toFixed(1)}km too far from target ${targetDistance.toFixed(1)}km`);
