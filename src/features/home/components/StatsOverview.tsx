...
export default function StatsOverview({ 
  ...
  // Read level from tracks (source of truth), fallback to domains
  const { domainLevel } = useMemo(() => {
    const tracks = profile?.progression?.tracks;
    const domains = profile?.progression?.domains;
  ...
  }, [primaryDomainId, profile?.progression?.tracks, profile?.progression?.domains]);
