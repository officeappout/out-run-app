...
export class ContextualEngine {
   ...
   */
  /**
   * Resolve the ExecutionMethod for an exercise in this filter context.
   * DELEGATES to the single-source selector `selectMethodForContext`
   * (../shared/utils/method-selection.utils), which every pipeline path now
   * shares so pool-filtering, guarantee/david refill, warmup, cooldown,
   * domain-rescue and recovery all pick a method the SAME way.
   *
   * `constraints` is retained in the signature for the caller but is no longer
   * read here: park gating is derived from the location alone (park is the only
   * location that carried `bypassLimits === true`, so `location === 'park'` is
   * exactly equivalent to the former guard). This delegation is byte-for-byte
   * behaviour-preserving vs. the previous inline implementation.
   */
  private findMatchingMethod(
    exercise: Exercise,
    context: ContextualFilterContext,
    _constraints: LocationConstraints
  ): ExecutionMethod | null {
    return selectMethodForContext(exercise, context.location, context.availableEquipment);
  }
