...
//
// REVERTED TO FALSE for the dormant merge to main (18.08.2026) — this flag
// was briefly staged true during the autonomous city-enrichment build run's
// own Phase A work (unpushed branch only, never deployed), but the merge to
// main is required to change ZERO live behavior. Flipping this true is one
// of the two explicit hard-stop actions gated on David's go — see the
// route-enrichment-pipeline plan doc's "Consolidated Destructive/
// Irreversible-Step Register" and the final activation checklist. Do not
// flip this true without that explicit approval.
...
// While false: the map "+" is BYTE-IDENTICAL to the pre-Phase-1 ActionSpeedDial
...
//   otherwise:     hasSchedule && hasAnsweredPersona(profile)
// (De Morgan's dual of the plan's own "gate is open" formula,
...
// underneath is deliberately out of this flag's concern, and deliberately
// not restated here — see persona-declaration.ts, owned by a different,
// separately-in-flight branch of work. Naming a specific field in this
// comment is exactly how the old version went stale the moment that file
