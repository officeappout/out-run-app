...
כאשר שני דומיינים מתחרים על אותו יום, הדומיין הדומיננטי מקבל עדיפות.
הדומיננטי הוא הדומיין שהמשתמש נרשם בשבילו — lifestyle.primaryTrack.
אין ליצור שדה חדש. השדה קיים ומוגן על ידי המשמר שנוסף בקומיט 4eb899d2.

1. דומיננטיות מכריעה בהתנגשות בלבד. היא אינה מפחיתה את מספר
   האימונים שהדומיין המשני מקבל.
2. מוצא אחרון: אם לא קיים לוז חוקי כשימי הדומיננטי מקובעים,
   מותר להזיז גם יום של הדומיננטי, עם הסבר מנומק.
   היעדר לוז גרוע יותר מהזזה מוסברת.
3. המטרה ניתנת לשינוי במסך עריכת לוז — אותו מסך שמחזיק את טוגל
   המאמן החכם ואת הכניסה למגירה. השינוי חל מכאן והלאה בלבד,
   ואינו מגלגל אחורה לוז קיים.

### אין טבלאות חלוקה היברידיות

אסור להוסיף טבלה, מפה או קבוע שמגדיר מראש כיצד מתחלקים ימים
בין דומיינים לפי מספר ימים. דוגמה אסורה: "היברידי 3 ימים →
ריצה/כוח/ריצה".
טבלת ימים מועדפים בתוך דומיין אחד (כמו PREFERRED_DAYS בחוקי הכוח)
לגיטימית — היא קלט. האיסור הוא על טבלה חוצת-דומיינים.
החלוקה ההיברידית היא תמיד פלט של הרצת החוקים.

---

## ההמלצה נגזרת מרמה, לא ממספרים שנשמרו

המגירה מציעה שבוע מומלץ שנגזר מרמת המשתמש בכל דומיין —
רמת הכוח, ורמת הריצה (מתחיל, שיפור קצב, תוכנית מובנית) — ומהחוקים.
היא אינה כבולה למספר האימונים שסומן בשאלון.

מעל ההמלצה פקד אחד: מספר הימים שהמשתמש יכול להתאמן בפועל.
שינויו מריץ מחדש את החוקים ומעדכן את ההצעה מיד.
מיקומו ההתחלתי הוא הסכום שסומן בשאלונים — נקודת פתיחה, לא אילוץ.
אין להוסיף שאלה חדשה בשאלונים על ימים פנויים.

שלושה סייגים:
1. תוכנית ריצה מובנית פעילה קובעת את מספר הריצות. המגירה רשאית
   להזיז ימי ריצה, לא לשנות את מספרן. שינוי כזה הוא החלפת תוכנית.
2. אם ההמלצה שונה ממה שסומן — שורת הסבר עם שני המספרים ואפשרות
   לחזור למסומן. שינוי שקט אסור.
3. בצמצום, הדומיננטיות מכריעה מי מצטמצם, ומוצג הסבר מפורש.
   צמצום שקט אסור.

---

## סולם השינוי: הזזה, הורדה, שינוי תוכנית

1. הזזה — אותו אימון, יום אחר, אותו שבוע. מספר האימונים לא משתנה.
   התוכנית שלמה, התחזית לא משתנה. לא מוצגת שאלה. אם היום החדש
   מפר חוק — הערה קצרה, ללא חסימה (ת5).
2. הורדה לשבוע הזה — אימון יורד בשבוע הנוכחי בלבד. אורך התוכנית
   נשמר, שבועות עתידיים לא משתנים, התחזית מתעדכנת.
3. שינוי תוכנית — מספר האימונים משתנה מכאן והלאה. דורש אישור
   מפורש והצגת התחזית המעודכנת לפני האישור.

המגירה התחתונה ("רק השבוע הזה" / "מכאן והלאה") עולה רק בהורדה
ובשינוי. לעולם לא בהזזה — הזזה היא הפעולה הנפוצה ביותר,
ושאלה עליה תהפוך את המסך למטרד.

### מי נופל בהורדה

המנוע מחליט, לא המשתמש מרשימה ולא "האחרון בשבוע".
לפי סדר קריטיות שמשפחת החוקים של הדומיין מגדירה.
בריצה: ריצה ארוכה ואימון איכות מוגנים; אימון קל ראשון לוותר עליו.
סדר הקריטיות המדויק ייכתב במשפחת חוקי הריצה, לא כאן.
בכוח: אותו עיקרון לפי חוקי הכוח.
בכל הורדה מוצג הסבר: מה ירד ומה נשמר.

### כלליות המנגנון

המנגנון אינו ייחודי לריצה-כוח. אותו מודול מתאם משרת גם שילוב של
תוכניות כוח שונות ושל סקילים. כל דומיין תורם משפחת חוקים אחת:
ימים מועדפים, מרווחים, סדר קריטיות. המודול המתאם אינו יודע דבר
על תוכן הדומיין. הוספת סקיל = הוספת משפחה, לא שכתוב.

### אזור הערות המאמן

כל ההסברים המחויבים מתנקזים לאזור אחד: צמצום מדומיננטיות,
הזזה מחוק, אימון שירד, פער בין המלצה למסומן.
בית אחד, לא הודעות מפוזרות.
...
עדכון: התוצאה שהמגירה בונה נגזרת מרמה ומהחוקים. הפקד הוא חלק
מהתרחיש.
...
### ת10 — עדיפות לפי מטרה
א': primaryTrack = ריצה, לוז ריצה פעיל, נוסף כוח. בהתנגשות —
הריצה נשארת, הכוח זז.
ב': primaryTrack = כוח. הפוך.
ג': אין פתרון חוקי עם הדומיננטי מקובע — מותר להזיזו, עם הסבר.
בדיקה: (א) מספר האימונים בכל דומיין נשמר. (ב) הזזת הדומיננטי
לעולם לא בשקט. (ג) תמיד נוצר לוז חוקי.

### ת11 — הפקד משנה את ההצעה
משתמש היברידי מקטין את מספר הימים. ההצעה נבנית מחדש לפי החוקים.
כל צמצום מלווה בהסבר. אם יש תוכנית ריצה מובנית ומספר הימים יורד
מתחת לנדרש בה — אזהרת החלפת תוכנית, ללא שינוי בלי אישור.
בדיקה: (א) אין צמצום בלי הסבר. (ב) תוכנית מובנית לא משתנה
בלי אישור. (ג) התוצאה חוקית.

### ת12 — שבוע 4, אפשר רק פעמיים
תוכנית ריצה מובנית, שבוע 4 מתוך 8, שלוש ריצות: ארוכה, איכות, קלה.
המשתמשת יכולה רק פעמיים. עולה המגירה התחתונה.
"רק השבוע": יורדת הקלה, הארוכה והאיכות נשמרות, התוכנית נשארת
8 שבועות, מוצג הסבר, התחזית מתעדכנת.
"מכאן והלאה": מוצגת התחזית המעודכנת, נדרש אישור מפורש.
בדיקה: (א) אימוני מפתח לא יורדים כל עוד יש פחות קריטי.
(ב) "רק השבוע" לא נוגע בשבועות עתידיים. (ג) "מכאן והלאה"
לא מתבצע בלי אישור. (ד) גרירה ליום אחר לא מעלה מגירה תחתונה.
