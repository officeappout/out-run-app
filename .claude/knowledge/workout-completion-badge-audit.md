...
## §5.4 — הוחלט: סכימת-hybrid + גרדיאנט דינמי (לא צבע קבוע), 22.08.2026 — **תוקן 22.08.2026 (מאוחר יותר)**

**דוד דחה את שתי האופציות שהוצעו וקבע גרסה שלישית, מאומתת-קוד ומדויקת:**

1. **⚠️ תיקון (דוד, אחרי אישור התוכנית):** **לא** לגעת ב-`ScheduleEntryType`/`entry.type` בכלל — לשדה הזה רק 3 ערכים (`'training'|'rest'|'assessment'`), והוא מתאר **האם** זה יום-אימון, לא **איזה סוג** אימון. hybrid entry נשאר `type:'training'`, בדיוק כמו strength/cardio. הניסוח הקודם ("`entry.type='hybrid'`") הועתק בטעות מהטקסט המקורי בלי להצליב מול `schedule.types.ts` — לתקן. **הוסיפו `'hybrid'` רק ל-`scheduledCategories` (וגם ל-`BuilderWorkoutType` איפה שזרימת ה-builder צריכה את זה)** — זה השדה שבאמת מבחין strength/cardio/hybrid.
...
### ✅ Stage A — בנוי (22.08.2026), עבר code-review עצמאי (PASS, 0 blocking, 2 warnings לא-חוסמים)

worktree: `feat+planner-completion-badge`. 2 commits:
1. `4a48781a` — סכימת-hybrid (`scheduledCategories`, `aerobicShare`) + גרדיאנט דינמי בכרטיס + `hybrid-colors.ts` הועבר ל-`src/lib` (תיקון cross-domain).
2. `86df851f` — `completeCommunityEntry`/`completeHybridEntry` (קישור-entryId מדויק) + חיווט ב-`finishWorkout`/`finishHybrid` + תיקון-שביריות ריצה-מתוכנית + נעילת-גרירה+lock-icon.

**§6 step 4 (איחוד-איתות ב-AgendaDayCard) התברר כמיותר ל-Stage A**: `AgendaDayCard` כבר קורא `entry.completed`/`e.completed` ישירות — זה תמיד היה נכון, רק חסר לו writer אמיתי. עכשיו יש writer (הcommit השני), אז אין צורך ב-bridge ל-`dailyProgress` בשביל #2/#3/#5. ה-bridge עדיין נדרש ל-Stage B (#1/#4, שאין להם entry כלל).

**2 אזהרות לא-חוסמות מה-review** (תיעוד, לא-דחוף): (א) `_userId` param ב-`completeCommunityEntry`/`completeHybridEntry` מתעלם בפועל (מוסכמה קיימת בקובץ, לא באג חדש). (ב) `sessionStorage.removeItem('planned_run_week'/'day')` ב-`useRunningPlayer.ts` יורה גם אם `activeProgram` עדיין לא נטען (race נדיר) — impact נמוך, `runMode==='plan'` gate מונע נזק בפועל.

**⚠️ לא נבדק על מכשיר** — TSC נקי + code-review PASS, אבל שום QA-חי לא בוצע (axioms §11 אוסר עליי להריץ `npm run dev`). דוד צריך לבדוק בפועל: התחלת+סיום אימון קבוצתי, אימון היברידי, ריצה-מתוכנית שננטשה-ואז-נחזרה, לפני push.
