...
## Verification — full Haifa dry-run (19.08.2026, round 2)

Live run, dry-run throughout, `--include-pending` set. Total wall-clock: 923.3s (~15.4 min —
longer than round 1's 312s, mostly Overpass retry variability on the climb steps, not a code
change; one full run this round hit a transient all-3-mirrors 504 on step 2 and was simply
retried, succeeding cleanly the second time — normal public-API flakiness, not a bug).

**A real, confirmed data-quality bug was found by independent review and fixed before this run
counted for anything**: the shared `computeDemProfile()` function silently returned a fake
`{gain:0, grade:0}` instead of `null` for any path shorter than its 15m resample step — meaning
short street segments would have been written as "verified flat" when they were never actually
measured. Fixed (`993187f4`) and re-verified against real TLV data: the earlier claimed
"1144/1144, 100% coverage" was wrong — real coverage is **688/1144 (60.1%)**, the other 456 are
now correctly reported as coverage gaps instead of fake zeros.

### Routes (step 1, `geo-discovery-routes.ts --region=haifa --dry-run`)

**69 kept**: 33 trail, 3 loop, 28 named segment, 5 cycling. 31 dropped outside boundary, 1 artifact.
(Unchanged from round 1 — no route-discovery code changed this round.)

### Climbs (steps 6–7, terrain discovery + write)

| Type | Kept | Dropped — outside boundary | Dropped — other |
|---|---|---|---|
| terrain | 213 | 47 | 3 artifacts (over private land) |
| structure | 43 | 13 | — |
| stairs | 491 | 174 | 5 escalators (conveying=yes, correctly excluded) |
| **Total** | **747** | **234** | 8 |

263 terrain candidates isolated before validation/clip. 638/747 needed reverse-geocoded names,
all resolved. `authorityId` resolved live end to end. (Small run-to-run drift from round 1's 758 —
live OSM data changes between runs, same pattern noted throughout this session, not a bug.)

### Amenities incl. crossings (step 10) — the big one this round

**2691 total candidates** (was 584 before crossings were added — a **4.6× jump**):
...
**276 dropped as outside the real municipal boundary** (new boundary-clip, mostly crossings —
proof this is doing real, substantial work, not a no-op: a rectangular bbox naturally sweeps in
many neighboring-municipality crossings/points that the real polygon correctly excludes). 27
suppressed by the garden-dedup gate, 2664 would be written fresh `pending`. Crossings alone are
82% of all candidates — pedestrian crossings are simply far more numerous in real OSM data than
courts/benches/fitness stations combined, exactly as expected for a real city.

### Street-segment elevation (step 5)

Reported **0 found — nothing to do** for Haifa, correctly: street segments were never actually
written this run (route discovery and the street-segment import steps are dry-run only), so
there's nothing yet for this step to grade — same dependency-chain shape as every other
downstream step. **Proven separately against real TLV data** (688/1144 segments, 60.1% real
coverage, 0.02s sampling time against an already-warmed cache) — see the fix note above.

### Everything else

- DEM tile-cache warm: 49 tiles requested, 0 cached, 49 would fetch.
- Street segments — calm: 9492 fetched, 6777 kept. Arterial: 484 fetched, 357 kept (190
  primary/167 secondary, avg flowScore 9.06). Unchanged from round 1.
- DEM elevation (routes), lit-tag, route-enrichment join, route-adjacency: all correctly report
  **0** for Haifa — expected, same dependency-chain reasoning as round 1 (nothing upstream was
  actually written). Route-adjacency's city list (זכרון יעקב, תל אביב-יפו, שדרות — 88 edges, 54
  published corridors) confirms it's genuinely city-agnostic with simply nothing for Haifa yet.
