...
> **עדכון לאחר בדיקה על מכשיר (08.08)**: דוד מצא כשל אמיתי — לולאה רגילה (בלי כתובת) של 22 ק"מ החזירה "לא הצליח למצוא מסלול". שורש הבעיה: חלון קבלת-המרחק `[target-0.5, target+2.5]` (שורה 36 למטה) הוא רוחב **מוחלט קבוע** של 3 ק"מ — 100% מיעד של 3 ק"מ, אבל רק ~14% מיעד של 22 ק"מ. תוקן: `computeDistanceWindow()` (הוצא לפונקציה טהורה + טסטים, `route-generator.service.ts`) — הטווח עכשיו סקיילי (10%/15% מהיעד, עם הערכים המקוריים כרצפה) ולא נשאר קבוע. ראו commit `7f9bae0`. תיקון isSafe/idealWaypointDistanceKm למטה טיפל בבעיה נפרדת (waypoint scoring) — זו הייתה בעיה שלישית, נפרדת, באותו "family" של "קבועים מוחלטים שלא סקיילים ליעד גדול".
> נבדק גם: האם מסלולים קיוריטד/מסומני-איכות מתעלמים — **לא**. מקטעי-רחוב רשמיים/מאושרי-אדמין כבר מקבלים בוסט לניקוד מקסימלי (10) ושולטים ב-top-12 (`route-generator.service.ts:346-354`), בלתי-תלוי בבאג הזה. אין משאב "מסלול-לולאה קיוריטד מוכן" נפרד שמתעלמים ממנו — `route-stitching.service.ts`'s `curated_routes` נשאר מנותק לגמרי מ-Free Run החי (אומת שוב), ואינטגרציה איתו תהיה משימה עצמאית נפרדת אם ירצו בכך.
