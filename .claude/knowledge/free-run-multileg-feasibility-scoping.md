...
**מחקר settle-preview (08.08, לא נבנה — המלצה בלבד)**: settle-preview של hybrid (`HybridSlotCarousel.tsx:308-323` → `handleSettleSlot` ב-DiscoverLayer.tsx:695-766) הוא לא "20 שורות" — מנגנון של ~200+ שורות שנצבר על פני כמה סבבי תיקון-באגים: debounce (300ms) + compose אמיתי מול נתוני-פארק חיים (`composeFullParkRoutePreview`/`composeHybridPlan`) + caches ייעודיים (`hybridRoutePreviewCache`, `hybridPlanCache`) + flow-id race guards + chip "מחשב מסלול...". חוזר-הפיך לגמרי (`startRun` הוא no-op בזמן preview, `resetHybridFlow` מנקה הכל). הצורה הכללית (debounce → compose → `setFocusedRoute` → MapShell מצייר) ניתנת לשימוש-חוזר עקרונית, אבל compose הבפועל, ה-caches, וה-flow-id guards צמודים-hybrid — leg-plan preview יזדקק ל-compose משלו (בעצם: `compileLegPlan` שכבר קיים!) ול-trigger משלו (אין carousel ב-UI של leg-plan). **המלצה**: לא לבנות עותק של כל מנגנון ה-settle — במקום זה, גרסה קלה בהרבה שמנצלת מה שכבר קיים: אחרי `compileLegPlan` מצליח (בתוך `handleStartRun` הקיים ב-LegPlanSheet), במקום לקרוא ל-`onStartLegPlanRun` מיד — לצייר את ה-Route המורכב על המפה (`logic.setFocusedRoute(compiled.route)`, כבר קיים ומוכן) ולהמתין ל-1-2 שניות (או ללחיצת-אישור קטנה) לפני שקוראים ל-`startActiveWorkout()`. אין carousel, אין cache, אין flow-id — רק "צייר, השהה קצת, התחל". היקף מוערך: קטן (יומיים-שלושה של settle-preview המלא → זה כמעט פשוט "תוסיף שורה שמציירת + delay/אישור" כי compileLegPlan וה-setFocusedRoute כבר קיימים משלב 2). לא נבנה — ממתין להחלטת דוד.
