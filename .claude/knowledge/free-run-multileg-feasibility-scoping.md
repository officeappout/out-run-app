...
**דיווח דוד אחרי בדיקה נוספת (09.08)**: settle-preview-lite + תיקון-הכפתור (`f35a3645`) לא הספיקו — הכרטיס לא נתן למסלול "במה" ראויה. שני ממצאים נוספים, ממוקדים, מאושרים לבנייה:

**✅ יעד-כתובת: אישור-לפני-commute, נבנה+נבדק, נדחף לענף review (09.08, commit `44aaf4a8`)** — בחירת כתובת ב-"📍 יעד: כתובת" בנתה מסלול מיידית (ישר ל-RouteCarousel, בלי שלב-בדיקה). store חדש `usePendingAddressStore.ts` (Zustand cross-remount, מבנה מקביל ל-useLegPlanStore אך **בכוונה לא אותו store** — איחוד שתי הזרימות ("החזון של מחולל-מאוחד") נדחה במפורש לסבב-תכנון נפרד). `onRequestAddressDestination` מפעיל `startComposing()` לפני פתיחת החיפוש; `handleAddressSelect` שומר את הבחירה ב-pending במקום לקרוא ל-`startCommute`; המגירה חוזרת עם "🏁 התחל לכתובת הזו" (אישור מפורש) / "ביטול". בחירות מסרגל-החיפוש הראשי (לא קשור למגירה) לא נגעו — עדיין commute מיידי כמו קודם.

**✅ LegPlanSheet: שמור-והמשך מול נקה-הכל, נבנה+נבדק, נדחף לענף review (09.08, אותו commit)** — X / הקשה-על-הרקע / גרירה-לסגירה שיתפו את אותו `onClose` שהחזיר ישר למגירה הראשית בלי קישור ל-`goalSheetOpen` — הרגיש כמו "זה מחק את העצירות?" גם כשהן נשמרו בפועל. `onClose` עכשיו פותח מחדש את GoalSheet — "שמור והמשך" חד-משמעי, נבדל מ-"נקה הכל" (כפתור נפרד קיים, `handleClearAll`, ללא שינוי).

TSC נקי (906, אפס שגיאות חדשות ב-4 קבצים), 468/469 בסוויטה (7 טסטים חדשים ל-usePendingAddressStore, כולם עוברים; אותם 3 כשלים קיימים-מראש). **דוד בדק בעצמו את שני התיקונים ואישר שהם תקינים** — ענף `review/leg-plan-address-confirm-09-08` (לא main, לא merged), ממתין ל-device-test ואז merge.
