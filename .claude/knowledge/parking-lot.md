...
## מחלקת-באג: "לא יודע" הופך ל"לא" — נמצאה ארבע פעמים בשני ימים
**Opened:** 2026-09-07 · **Source:** דוד, אחרי סבב-חקירה על הצהרת-בריאות-נשאלת-שוב + מנעול-לא-מרונדר — "נסח את זה כמחלקת באג, לא כשלושה באגים." **עודכן 08.09.2026** — מופע רביעי, `mapAnswersToProfile`, נמצא כשורש-הבאג של #3 עצמו.

ארבעה מקומות, ארבעה תחומי-קוד שונים, אותה צורה בדיוק: קוד שצריך לייצג שלושה מצבים (כן / לא / עדיין-לא-ידוע) מקבל רק שני ערכים אפשריים — ו"עדיין לא ידוע" נבלע לתוך אחד הקצוות, כמעט תמיד "לא"/"ברירת-המחדל-הבטוחה-כביכול". התוצאה בכל הפעמים: לא קריסה, לא שגיאה גלויה — התנהגות *שקטה* ושגויה שנראית תקינה עד שבודקים אותה נגד המצב האמיתי.
...
4. **`mapAnswersToProfile`** (`profile.service.ts:135-365`, שורש הבאג שלוש-ה-3 נבדקו בזכותו — נמצא 08.09.2026) — **הצורה הכי טהורה של המחלקה, לא עוד מופע.** זו לא פונקציית-מיפוי — זו פונקציית-**בניית-פרופיל-חדש**, שממלאת כל שדה שהשאלון-הנוכחי לא כיסה בברירת-מחדל: `'משתמש'`, `70` (ק"ג), `'other'`, `'none'`, `globalXP=0`/`globalLevel=1` קשיחים, `lifestyle`/`equipment`/`running` ריקים-לגמרי. למשתמש-ראשון זו התנהגות לגיטימית — אין "ישן" להתבלבל מולו. **מופעלת על משתמש-קיים** (`dynamic/page.tsx:434`, בכל מעבר-onboarding נוסף, כולל הוספת-מסלול-שני) — ואז כל ברירת-מחדל הופכת למחיקה: "לא שאלתי על זה הפעם" נכתב כאילו הוא "הערך האמיתי הוא ברירת-המחדל". שישה שדות לפחות (`core`, `progression`, `equipment`, `lifestyle`, `health`, `running`) חשופים במלואם — פונקציה שכל תפקידה הוא להמציא ודאות, מופעלת בדיוק במקום שבו יש כבר ודאות אמיתית לאבד.
...
## ✅ `SKILL_TO_FOUNDATION_OFFSET = 9` — נבדק מול דאטה אמיתי, זוכה. סגור.
**Opened:** 08.09.2026 · **Source:** דוד, מלוג-האבחון — "מספר קסם +9 בלי מקור נראה לעין," שודרג באותו יום ל"סיכון בטיחות." **נסגר באותו יום**, אחרי בדיקת-דאטה ישירה (לא הסקה מקוד): "הזוגות שהבאת מראים +9 עקבי... זה לא מספר קסם בקוד — זה השיוך שדוד בנה בפאנל. הקוד משקף אותו נאמנה."

`onboarding-sync.service.ts:77` — `const SKILL_TO_FOUNDATION_OFFSET = 9;`, עם הערת-קוד (שורות 60-74) שמצטטת מקור: *"Root scale relationship (from master training spreadsheets)"*, תחת הכותרת `"David's Matrix"`. הנוסחה: `Foundational Track Level = Assessed Skill Level + 9`. מופעל רק ל-Path C (סקילים-בלבד) שלא עברו הערכת-push/pull נפרדת.

**האימות — שאילתת Firestore ישירה על אוסף `exercises`, לא קוד, לא הסקה.** לכל תרגיל שנושא תג `planche` נבדק זוג `(planche level, push level)` מתוך `targetPrograms[]` שלו; אותו דבר ל-`one_arm_pullup` מול `pull`.

- **planche:** 31 תרגילים מתויגים. 30 מהם נושאים גם תג push — **כל ה-30 בדיוק +9** (planche level + 9 = push level, ללא יוצא מן הכלל). התרגיל ה-31 (`oLzPZ7rQMdOUgGuVr1os`, "פשיטת ירך אחורית") **אין לו תג push כלל** — נושא `legs L5` במקומו. זו לא סטייה מ-+9, זו היעדרות-תג — נושא נפרד, לא נבדק.
- **one_arm_pullup:** 26 תרגילים מתויגים, כולם נושאים תג pull. **25 מתוך 26 בדיוק +9.** חריג יחיד: `bYVShoP3T506soTMWxHO` ("מתח שכמות קשתים") — `oap L1 → pull L11`, כלומר **+10** ולא +9.

**מסקנה: ה-9 אינו מספר-קסם בקוד — הוא משקף בנאמנות את השיוך שדוד עצמו בנה בפאנל האדמין, כמעט-תמיד בדיוק.** לא "מוריד דרגה" — סגור, עם הנתונים.

**שורה אחת שנשארת פתוחה, לא לפתרון כאן:** החריג היחיד (`bYVShoP3T506soTMWxHO`, oap 1→pull 11) הוא אי-עקביות דאטה קטנה בתיוג הבודד הזה בפאנל, לא באג קוד. דוד יחליט אם לתקן בפאנל.

**⚠️ נקודת-צימוד, לא באג היום — נקודת-שבירה עתידית:** `SKILL_TO_FOUNDATION_OFFSET=9` **מקודד קבוע ב-TypeScript** (`onboarding-sync.service.ts:77`), **לא נגזר מהשיוך בפאנל בזמן ריצה**. הקוד לא קורא מה-`targetPrograms` בפועל כדי לחשב את ה-9 — הוא מניח אותו. כל עוד השיוך בפאנל שומר על +9 עקבי (כפי שאומת עכשיו), הקבוע נכון. **אם דוד ישנה בעתיד את השיוך בפאנל** (יתייג תרגיל planche חדש ברמת push שאינה +9 מרמת ה-planche שלו) — הקבוע `9` בקוד **לא ידע על כך ולא יתעדכן**, כי שום דבר לא מצמיד ביניהם. זה לא חוסם היום; זה תיעוד של איפה בדיוק ה-9 יושב, למי שיצטרך לזכור את זה בפעם הבאה שהשיוך בפאנל משתנה.

---

## שלוש אפשרויות-אימון זהות — "Single-option path — padded 3 slots with the same workout"
**Opened:** 08.09.2026 · **Source:** דוד, שתי ריצות-אבחון נפרדות, אותה תופעה בשתיהן.

כשהמנוע נכנס לנתיב-אפשרות-יחידה (single-option path), הוא ממלא שלושה slots בדיוק עם אותו אימון — לא שלוש הצעות שונות. חוזר עקבית, לא מקרה-בודד. לא נחקר לעומק, לא תוקן — פריט נפרד מסעיף ב'/ג' של אותו לוג, לא לערבב.
...
## אחרי תיקון `oap` — הסקילים נכנסים לבריכה, אבל עדיין מקבלים אפס תרגילים
**Opened:** 08.09.2026 · **Source:** דוד, ריצה B (פלאנץ'+מתח יד אחת) על ענף `diag/health-declaration-store-staleness`, אחרי אימות שתיקון ה-`oap` עבד.

`resolvedChildDomains`/`focusDomains` עכשיו נכונים (`[planche, one_arm_pullup, push, pull]`), הזיווג-אנטגוניסטי מצליח (1 push+pull pair). **אבל בשלב-בחירת-התרגילים עצמו:**
```
DOMAIN QUOTA FAILED: No exercise for "planche" (aliases: [push])
DOMAIN QUOTA FAILED: No exercise for "one_arm_pullup" (aliases: [])
planche L10: 7 sets ÷ 0 exercises = 0 sets/each
one_arm_pullup L10: 7 sets ÷ 0 exercises = 0 sets/each
```
האימון בפועל נבנה **כולו** מתרגילי-ההורים (push/pull) — משתמש שבחר פלאנץ' ומתח-יד-אחת קיבל פרונט-לבר ושכיבות-סמיכה-בעמידת-ידיים, אף לא תרגיל אחד שהוא בעצם "פלאנץ'" או "מתח יד אחת".

**⚠️ אי-עקביות בתוך אותו לוג, לא נחקרה:** ל-`planche` יש `aliases:[push]` (הורה אחד) — ל-`one_arm_pullup` יש `aliases:[]` (ריק לגמרי). שני סקילים, אותו מנגנון-כאורה, פלט שונה. **שאלה פתוחה — למה לסקיל אחד יש aliases ולשני אין. לא לחקור עכשיו.**

**✅ 08.09.2026 — הדאטה נבדקה: התרגילים היו זמינים. הבאג בקוד, לא בדאטה ולא במיפוי.** שאילתת Firestore ישירה (לא הסקה): המאגר של הריצה הזו הכיל תרגילים ברמות push/pull 16-20 בלבד. בתוך אותו חלון בדיוק נמצאו **10 תרגילים מתויגי-planche** (רמות push 16/16/20/19/20/18/17/17/16/18) **ו-10 תרגילים מתויגי-one_arm_pullup** (רמות pull 18/16/16/18/16/16/17/19/19/16) — עשרים תרגילים זמינים בחלון הנכון, מתוך 19 בסך-הכל במאגר. הכפילות-לכאורה (20 מתויגים מתוך 19 בסה"כ) נובעת מכך שאחדים נושאים גם תג-הורה נוסף (front_lever/handstand_pushup) לצד planche/oap. **המסקנה סגורה: לא שאלת מיפוי-רמות (זו נבדקה ונסגרה בפריט הקודם) ולא שאלת זמינות-דאטה — שאלת קוד.**

**✅ 08.09.2026 — תרגיל אחד, מסלול אחד, נקודה אחת, נמצאה ומאומתת (READ-ONLY, לא תוקן).**

תרגיל-בדיקה: `HSYxn9dL4d9nUImW3Y7G` ("לחיצה מישיבת L לפלנאץ׳ בטאק") — `targetPrograms: [{push, L16}, {planche, L7}]`, מאומת ישירות מה-Firestore doc. נמצא בדיוק בחלון push 16-20 שהריצה חיפשה בו.

**שני closures סמוכים באותו קובץ, `selectExercisesWithDomainQuotas`
(`src/features/workout-engine/logic/workout-selection.utils.ts:697-714`), מתנהגים אחרת אחד מהשני על אותו נתון:**

```ts
// :697-707 — הבאג. סורק tp אחד בכל פעם, מחזיר בהיפגעות ראשונה —
// גם התאמה-ישירה וגם התאמת-הורה נבדקות באותה איטרציה, בסדר המערך.
const getLevelForDomain = (ex: Exercise): number => {
  if (ex.targetPrograms) {
    for (const tp of ex.targetPrograms) {
      ...
      if (parentAliases.includes(slug)) return tp.level;   // ← נפגע כאן, ראשון
      if (tp.programId === domain) return tp.level;
    }
  }
  return ex.recommendedLevel ?? userLevel;
};
// :708-714 — לא הבאג. .some() בודק את כל המערך, לא תלוי-סדר.
...
עבור `HSYxn9dL4d9nUImW3Y7G` עם `domain='planche'` (`parentAliases=['push']`, מאומת מ-`DOMAIN_PARENT_MAP` שנבנה מ-`DOMAIN_ALIAS_MAP`'s `push:['planche','handstand']`, `:28-52`): ה-`targetPrograms` הראשון הוא push. `getLevelForDomain` בודק אותו — `slug==='push'`, `parentAliases.includes('push')===true` — **מחזיר 16 ויוצא מהלולאה, לפני שהגיע בכלל לרשומת ה-planche השנייה (L7).** `belongsToDomain`, שתי שורות מתחת, סורק את **כל** המערך ומזהה נכון שהתרגיל שייך ל-planche — **הזיהוי תקין, הרמה שגויה.**

התוצאה במורד-הזרם, שלושת שכבות-ההצלה (`:729-791`), עם `userLevel=userLevelsMap.get('planche')=10` (מאומת `:654`, הרמה הנבחנת הגולמית — לא ה-offset):
- שלב 1 (`:733`): `getLevelForDomain(ex)!==userLevel` → `16!==10` → נדחה.
- שלב 2 (`:743-764`): סורק `10±{1,2,3}`; `getLevelForDomain` מחזיר **תמיד 16** לתרגיל הזה (לא תלוי ב-delta) → אף התאמה → נדחה.
- שלב 3 (`:769-791`): `levelFloor=max(1,10-5)=5`, `levelCap=10+3=13`; `16>13` → נדחה.

שלושתם נכשלים, `rescue` נשאר `undefined`, ומופעל ה-`else` ב-`:812-815` — בדיוק ה-`DOMAIN QUOTA FAILED... (aliases: [push])` שנצפה בלוג. מאומת ישירות מהקוד (לא רק מהסוכן שסקר) — קראתי את `:690-820` בעצמי.

**היקף מפורש, כפי שהתבקש:** נבדק תרגיל-planche אחד בלבד, מסלול אחד. `getLevelForDomain`/`belongsToDomain` הם אותו מנגנון-בדיוק גם ל-`one_arm_pullup` (אין הבחנה בין הדומיינים בקוד הזה) — **סביר שאותו באג-בדיוק אחראי גם שם, אבל זה לא נבדק לתרגיל-oap קונקרטי בסבב הזה** — אל תניח את זה כמאומת עד שמישהו יעקוב אחרי תרגיל-oap אחד באותה שיטה.

לא נחקר לעומק מעבר לזה, לא תוקן — נפרד מתיקון-ה-`oap` עצמו (שתיקן את שלב ה*-זיהוי*, לא את שלב ה*-בחירה*).

---

## תרגילי-חימום נכנסים לאימון הראשי, בכפילות
**Opened:** 08.09.2026 · **Source:** דוד, אותה ריצת-אבחון (ריצה B).

רשימת-התרגילים-הראשית של האימון שנוצר כללה תרגילי-טבטה (סמוך קום, הליכות זחל, קפיצות כוכב, הליכות דוב) — **פעמיים ברצף**, בתוך ה-Main, לא רק ב-Warmup:
```
Exercises: [..., סמוך קום, הליכות זחל, קפיצות כוכב, הליכות דוב,
            סמוך קום, הליכות זחל, קפיצות כוכב, הליכות דוב]
Warmups: 3 | Main: 11
```
לא נחקר לעומק, לא תוקן.
