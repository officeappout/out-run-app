...
## כלל עבודה: בדוק חפיפה עם סשן מקביל לפני שמתחילים משימה — 25.09.2026

**Opened:** 25.09.2026 · **Source:** ביקורת ה-worktrees/branches השבועית (Sderot pilot audit). זה **כלל עבודה קבוע**, לא משימה לביצוע וסגירה.

שלוש פעמים באותו יום עבודה התברר שענף/עבודה שלמה היו מיותרים — כי **סשן מקביל אחר כבר עשה בדיוק את אותו דבר**, ואף אחד מהצדדים לא ידע על השני: `197ab45b`, §13.18, ואז `feat/route-editor-freehand-satellite` (הוכח בייט-לבייט זהה ל-`6cbb4df5` שכבר ב-main, כולל timestamp מדויק זהה). זה מחיר ישיר של הרצת מספר צ'אטים/סשנים פעילים על אותו ריפו בו-זמנית.

**הכלל:** לפני שמתחילים משימה לא-טריוויאלית (במיוחד אם השם/הנושא כללי — "route editor", "authority boundary", "amenities") —
```
git fetch origin main
git log origin/main --oneline --since=<תאריך פתיחת המשימה>
```
וחיפוש טקסטואלי בנושא (`git log --all --oneline -S"<מונח מפתח>"` או פשוט קריאת הכותרות). שלושים שניות שיכולות לחסוך יום שלם של עבודה כפולה — ולפעמים, כמו כאן, למנוע גם PR/merge מיותר שמישהו אחר יצטרך לגלות ולנקות אחר כך.

**מתי לא צריך:** משימה שכבר ידוע במפורש שאף אחד אחר לא נוגע בה (למשל, ענף ממוקד מאוד עם שם ייחודי, או כשדוד כבר אמר "אני עובד על X בצ'אט אחר, אל תיגע"). הכלל הזה נועד בדיוק למקרה ההפוך — כשאין שום איתות שמישהו אחר עלול לעבוד על אותו נושא, וזה בדיוק המקרה המסוכן.
