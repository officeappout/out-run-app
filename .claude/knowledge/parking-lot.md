...
**1. המגירה והראנר לא מסכימים על הסבר חלופי.** `MasterExerciseView.tsx:623-636` — נעול-לשיטה במפורש ("method-LOCKED... Never another method's media"), לא מציג הסבר-של-שיטה-אחרת בשום מצב. ה-CTA "צפה בהסבר המלא" בראנר (`ExerciseVideoPlayer.tsx:33`) כן מציג, דרך ה-`.reduce` חוצה-השיטות ב-`media-resolution.utils.ts` (`fullTutorial`, היה `:138-141` לפני `commit 1c93cfac` על ענף נפרד — התיקון שם קבע park-first במפורש, אבל **לא שינה את זה שהראנר בכלל נופל לשיטה-אחרת ואילו המגירה לא**). שתי התנהגויות מכוונות, מתועדות בקוד, וסותרות זו את זו. ממתין להכרעת דוד — איזו משתי ההתנהגויות נכונה, או שהפער בכוונה.
