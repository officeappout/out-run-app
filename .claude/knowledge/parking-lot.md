...
**Real, ongoing risk logged for whoever touches this next:** `cleanupEphemeralDocs.ts`'s sweep necessarily duplicates `community.service.ts`'s `deleteGroup()` contract by hand — Cloud Functions run on the Admin SDK and can't import the Next.js app's client-SDK service code. Two hand-synced implementations of one contract, in two SDKs. If the contract changes again (a new subcollection to clean, a new field to arrayRemove), both copies need updating — nothing enforces that today.

**"שמור" (save-only, no sharing intent) also creates one of these groups today** — confirmed, and explicitly named as unwanted. Not fixed yet: `route.ts`'s host-schedule-entry write (`addScheduleEntryAdmin`, the "האימון הבא" home-screen card) is keyed on `entryId: run_${groupId}` and stores `groupId`/`groupName` on the entry itself — it would not survive untouched if group-creation were skipped for this path. Needs the schedule entry given its own non-group identity (e.g. `entryId: solo_${uid}_${datePart}`) before "שמור" can stop creating a group. Small and contained, but a real change, not a no-op — blocked on David's go-ahead specifically for this piece.
