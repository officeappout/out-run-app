...
**רמז לשורש, מדוד דוד (16.09.2026, לפני revert) — התחל את החקירה הבאה כאן:** אחרי ה-revert, `equippedParksWithin`/`nearestEquippedPark` (`park-out-and-back.ts`) חזרו לבדוק `gymEquipment.length` הגולמי במקום `hasUsableEquipment`. **נקודת המעבר הזו (מסינון-לפי-דגל-מחושב חזרה לסינון-לפי-שדה-גולמי) היא החשוד המוביל**, לא שום דבר אחר שנבדק: בחירת-המועמד עברה מ-SPEC-07 לדגל `hasUsableEquipment`, וייתכן שמשהו בהמשך השרשרת (אחרי הבחירה, לפני/בתוך `extractParkEquipment` או מעבר בין הקטלוג לרשומה המלאה) המשיך להניח רשומה מלאה זמינה במקום לצפות לדגל בלבד. להתחיל את החקירה הבאה בדיוק שם — בגבול שבין "הקטלוג בחר את הפארק" ל"הפונקציה הבאה קיבלה אותו".
