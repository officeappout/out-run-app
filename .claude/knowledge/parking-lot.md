...
## `SKILL_TO_FOUNDATION_OFFSET = 9` — "מטריצת דוד", לא ממש חסר-מקור
**Opened:** 08.09.2026 · **Source:** דוד, מלוג-האבחון — "מספר קסם +9 בלי מקור נראה לעין."

`onboarding-sync.service.ts:77` — `const SKILL_TO_FOUNDATION_OFFSET = 9;`, עם הערת-קוד (שורות 60-74) שכן מצטטת מקור: *"Root scale relationship (from master training spreadsheets)"*, תחת הכותרת `"David's Matrix"`. הנוסחה: `Foundational Track Level = Assessed Skill Level + 9` (למשל: Planche L7 → Push L16). מופעל רק ל-Path C (סקילים-בלבד) שלא עברו הערכת-push/pull נפרדת.

**לא "בלי מקור" — אלא מקור-מיוחס-לדוד-עצמו שלא ניתן לאימות מהקוד.** ההערה קוראת לזה "David's Matrix" ומפנה ל"גיליונות-אימון-ראשיים" — לא קובץ, לא לינק, שום דבר שניתן לפתוח ולבדוק מכאן. אם זו באמת החלטה שלך מתקופה קודמת — שווה לאשר שהיא עדיין נכונה; אם לא — שווה למצוא מאיפה זה הגיע בפועל. לא נחקר לעומק, לא תוקן.
