...
## `users/{uid}.social.groupIds` vs `user_memberships/{uid}.groupIds` — no declared canonical source
**Opened:** 2026-09-08 · **Source:** the delete-contract incident — `social.groupIds` got wiped by a write-form bug, `user_memberships.groupIds` didn't, and the intact one was used to repair the broken one

Two collections hold the same fact (which groups a user belongs to), written together on join (`joinEngine.ts` 4b/4c, axiom §17) and — as of today's delete contract — cleaned together on delete. Nothing in the codebase declares which one is authoritative if they ever disagree. Today they diverged by accident (a bug in one write, not the other) and got noticed only because a manual content-check ("show me the content, not 'updated'") happened to compare them. If a future divergence is silent — no comparison, no reason to look — there's no rule saying which value wins, or even that they *should* match.

**Open question, not answered here:** is `user_memberships` canonical (it's the one Firestore rules' `memberGroupIds()` actually reads for presence authorization — arguably makes it the "real" one) and `social.groupIds` a client-read denormalized copy that should be treated as derived, rebuildable, and never hand-trusted? Or is there a reason it should be the other way, or genuinely neither? Whoever answers this should also decide whether a periodic reconciliation job is worth building, or whether "compare on suspicion, trust neither blindly" stays the working posture. Not scoped, not built — recording the question so the next divergence isn't a surprise.

---

## Standing rule: verify a new write form on a synthetic doc before it touches a real one
**Opened:** 2026-09-08 · **Source:** David, after the delete-contract's `social.groupIds` write corrupted 5 real users — the fix was verified synthetically only *after* the damage, in the wrong order

Whenever a Firestore write uses a shape that hasn't been exercised in production before (a new nested-path pattern, a new combination of `merge`/`mergeFields`/`update`, a new FieldValue sentinel in a new position), verify it against a synthetic, disposable document first — create it, apply the exact write, inspect the result, delete the fixture — before applying the same write to any real document. This is not optional caution for "risky-looking" writes only; the incident that produced this rule looked like a safe, symmetrical mirror of an already-working pattern (`joinEngine.ts`'s arrayUnion write, reversed to arrayRemove) and still broke a real nested-field write silently. Symmetry with working code is not proof a new write form works — the client SDK's `set()`/`merge` and `update()` handle dotted/nested paths differently, and that difference doesn't announce itself until something reads the actual content afterward.

---

## Workout-sharing creates a `community_groups` doc — root-caused, migration deliberately deferred to post-launch
**Opened:** 2026-09-08 · **Source:** full root-cause investigation (Explore agent) + David's decision on it, same day

**Where and why:** the only creation path is `src/app/api/invite/run-session/route.ts` — it never calls `createGroup()`, writes directly via the Admin SDK bypassing both the service layer and `firestore.rules`' `source`-required check (which is why every one of these docs was born without a `source` field). It's deliberate reuse of already-built plumbing: the join engine, the `attendance` subcollection (live session state — phase, attendees, progress), the `user_memberships` presence gate for map visibility, and the join landing page. A `type:'ephemeral'` flag was meant to keep these out of real group UIs — enforced in exactly one place (a leaderboard filter) before today, the fifth confirmed instance this session of a flag written but not checked everywhere.

**What's genuinely load-bearing (would break if the group entity were simply removed):** the live session's `attendance`/`member_statuses` state (physically a subcollection of the group doc); the only "someone joined your run" push notification in the app (keyed on the group's `members` subcollection write); host identification in the session lobby (`community_groups/{id}.createdBy`); the post-run "we ran together" summary (attendee profiles + group name).

**Confirmed NOT load-bearing:** two-runner map presence (reads `user_memberships` only, never the group doc); no counter anywhere depends on this (the related achievement is hardcoded to `stats.groupsJoined: 0` and can never fire); no chat thread is ever created on this path; GDPR account-purge already targets these by `createdBy` and just needs re-pointing if the entity changes shape.

**The `source:'user'`/`'authority'` split is fully explained, not arbitrary:** a later backfill script (`scripts/backfill-legacy-group-source.ts`) classified every sourceless doc — these included — by whether its creator was an admin account. Meaningless for this entity; a scope gap in a one-time script, not randomness.

**A real replacement already exists, ~90% built:** `planned_sessions` + `functions/src/onPlannedActivityCreated.ts` (partner/radius visibility via `connections.followers`/geohash, its own push channel, automatic TTL cleanup), reachable today behind `SOCIAL_COMPOSE_UI_ENABLED`, rendered right next to the group-creating share button in `RunShareBar.tsx`. This is the "broadcast, not a club" shape.

**David's decision (08.09.2026): do not migrate now.** Moving live-session infrastructure (presence, notifications, lobby, summary) onto `planned_sessions` the week before app-store launch is too much risk for a problem whose visible symptom (UI pollution) has a near-zero-risk fix. **Revisit after launch, using this entry + the full agent report (in this session's transcript, 08.09.2026) as the starting brief — do not re-investigate from scratch.**

**What shipped today instead, all zero-risk (pure filters + a cleanup sweep, no change to the share mechanism):**
1. `type !== 'ephemeral'` enforced in `getMyGroups()` (`src/features/arena/services/group.service.ts`, feeds both `CommunityCircles` and search's "שלי" mode) and `getAllGroupsForAdmin()` (`src/features/admin/services/community.service.ts`) — the two functions behind all four leaking UI surfaces.
2. The delete contract itself extended to also clean up the `attendance` + `member_statuses` subcollections (harmless no-op for real groups, which never have one) — needed before any automated cleanup of ephemeral groups could be built without creating a new orphan class.
3. `functions/src/cleanupEphemeralDocs.ts` now sweeps expired `type:'ephemeral'` groups hourly, keyed on `group_invitations.expiresAt` (not the group's own age — a scheduled run can be far in the future). **NOT yet deployed** — needs its own explicit deploy approval, same as the two new `group_invitations` composite indexes it requires (`firestore.indexes.json`, added but not deployed).

**Resolved 08.09.2026, second round — not left as an unenforced risk.** David rejected both easy answers: an HTTP call from the Cloud Function to a shared API route (a secret to manage, a network hop that can fail mid-delete, cross-service coupling for one DB write), and accepting the duplication as-is ("exactly the flaw we spent today on — four places deleting four different things, don't make it five"). A real shared TypeScript module was tested empirically: widening `functions/tsconfig.json`'s `include` to reach `src/` outside `functions/` does compile, but TypeScript's `rootDir` inference then restructures ALL of `functions/lib`'s output (`index.js` moves from `lib/index.js` to `lib/functions/src/index.js`), silently breaking `package.json`'s `main` entry point for every deployed function — confirmed by actually running the build, not assumed. A non-disruptive version (a real local npm package with TypeScript project references, built independently) is possible but is new infrastructure, not a same-day change.

**What shipped instead:** `deleteGroupContractAdminSdk()` (`functions/src/cleanupEphemeralDocs.ts`) stays a hand-written, separate implementation — but `tests/group-delete-contract-parity.ts` runs it and the client-SDK `deleteGroup()` against structurally identical synthetic fixtures and asserts the resulting state matches exactly. A future change to one implementation that isn't mirrored in the other fails this test instead of silently drifting. David's own condition: "בלי הטסט — לא. עם הטסט — מקובל" (without the test, no; with the test, acceptable) — logged here as the conscious, tested debt, not an unenforced one. Re-run `npx tsx tests/group-delete-contract-parity.ts` after any change to either `deleteGroup()` or `deleteGroupContractAdminSdk()`.
...
## שלושת נקודות-הלוג (MEV/SMFC/COOLDOWN-NUKE) — קבועות בפרודקשן, לא זמניות, לא מועמדות לניקוי — 16.09.2026

**Opened:** 16.09.2026 · **Source:** דוד, באישור מיזוג `diag/execution-method-identity-log` לפרודקשן: "הלוגים נשארים בפרודקשן לצמיתות — לא זמניים... הם התרעה על החלפה שקטה, וזה בדיוק מה שהמערכת הזו הייתה חסרה."

**החלטה: אל תסיר, אל תתייחס אליהן כ-diagnostic-זמני שממתין לניקוי.** שלוש הנקודות (כולן pass-through טהור — עוברות-ערך, אפס שינוי-התנהגות; תוכננו ב-`execution-method-identity-plan.md` Stage 1, אושר ע"י דוד):

1. `[MEV-MISMATCH]` — `MasterExerciseView.tsx` (`computeSeedIdx`) — שכבת-התצוגה (וידאו-תרגיל + drawer-תצוגה-מקדימה של אימון) נחתה על שיטת-ביצוע שאינה התאמה מדויקת למיקום המבוקש.
2. `[SMFC-MISMATCH]` — `method-selection.utils.ts` (`selectMethodForContext`, ה-helper `logMismatch`) — המנוע עצמו, **בזמן-יצירת-האימון**, נחת על שיטת-fallback לא-מדויקת (home-fallback / available-gear / home-park-rescue / bodyweight-only).
3. `[COOLDOWN-NUKE]` — `cooldown.service.ts` (`appendCooldownExercises`) — ה-fallback המוחלט של cooldown: מאגר ה-cooldown המסונן-למיקום היה ריק, נבחר תרגיל מהמאגר-המלא בלי קשר למיקום/ציוד, ומצוין אם הוטבע placeholder קשיח (`{location:'home', media:{}}`) או נמצאה שיטה אמיתית.

**נימוק דוד, במפורש:** אלה לא לוגים לצורך דיבוג חד-פעמי שממתין לתשובה ואז מסתיים. הם ההתרעה על **בדיוק** התקלה שהמערכת הזו לא הייתה מסוגלת לחשוף בעצמה: תרגיל שמוחלף בשקט — בלי סימן, בלי כשל-קשיח, בלי מי ששם לב. הסרתם (אפילו בקומיט-ניקוי גנרי, "diagnostic cleanup") תסגור בחזרה בדיוק את החור הזה.
...
## הדליפה של Priority-3 היא נושאת-משקל — אסור לתקן לפני שיש תוכן מתויג — 16.09.2026

**Opened:** 16.09.2026 · **Source:** מדידת כיסוי-מיקומים מתוקנת (execution-method-identity), אחרי שני סבבי-תיקון-מתודולוגיה. נרשם לפי בקשת דוד, לא תוקן.

`content_gap=0` בתשעה מתוך עשרה `ExecutionLocation` (הכל חוץ מ-`park`) הוא **לא** אות ש"אין בעיית-תוכן" — הוא תוצר-לוואי ישיר של `method-selection.utils.ts` Priority 2.5 (`:209`, "כל שיטה שדורשת רק ציוד-זמין") ו-Priority 3 (`:225`, "בודי-וויט-בלבד, כל שיטה, בכל מיקום") — שתי הדרגות האלה **לא בודקות תיוג-מיקום בכלל**, רק ציוד. כל תרגיל שיש לו **שיטת בודי-וויט אחת בודדת בכל הקטלוג** (לרוב מתויגת `park`, כי שם רוב התוכן נוצר) **נבחר כשיר** גם עבור `home`/`office`/`school`/`library`/`desk`/`airport`/`service`/`street`/`gym` — ללא קשר לתיוג בפועל של השיטה למיקום הזה.

**`park` הוא היחיד שחושף אמת:** יש לו שער-מיקום נוקשה (`:151` ואילך, "PARK: Strict location enforcement" — אין נפילה ל-home, אין נפילה ל-Priority 2.5/3 הכלליים) — ולכן `park` הוא היחיד שבאמת מראה פער-תוכן ממשי (56 תרגילים ללא שום שיטת-פארק, ראו מדידה קודמת). בתשעת המיקומים האחרים, הדליפה **כבר "סוגרת" את הפער הזה בפועל היום** — לא כי התוכן תויג, אלא כי הבחירה עוקפת את התיוג.

**אסור לתקן את הדליפה לפני שיש תוכן מתויג לתשעת המיקומים.** אם Priority 2.5/3 ייסגרו (יתחילו לדרוש תיוג-מיקום מדויק, לא רק ציוד) לפני שנוצר תוכן מתויג אמיתי ל-`home`/`office`/`school`/... — תשעת המיקומים **ייפלו בו-זמנית** לפער-תוכן חמור, באותו רגע, כי היום שום דבר לא באמת מתויג להם מלבד `park`/`service` (וגם `service` יורש את אותה דליפה — ראה מדידה מתוקנת, טבלת ה-window שלו זהה ל-`home`). **סדר מחייב: תוכן → תיוג → סגירת הדליפה.** הפוך את הסדר — ותהיה נסיגה חדה וגלויה בו-רגע לכל תשתית האימונים מחוץ לפארק.

**הערה נלווית, מאותה סדרת-תיקונים:** נמצא גם fallback דומה-באופי בשכבת-התצוגה — `resolveExerciseMedia` (`media-resolution.utils.ts:106-118`) מחפש וידאו בכל שיטה של התרגיל, לא רק בשיטה-שנבחרה, ואז ברמת-התרגיל. זה מה שהופך כיסוי-וידאו "אמיתי" מ-58.5% (בית, ברמת-שיטה-בלבד) ל-99.1% (בית, כפי שבפועל מוצג למשתמש) — אותה תבנית בדיוק (fallback-רחב שמסתיר פער-תיוג-אמיתי), שכבה אחרת. לא תוקן, לא נבדק אם צריך.

---

## שלוש אופציות ה-trio הן אותו אובייקט ברפרנס — לא רק "תוכן דומה" — 16.09.2026

**Opened:** 16.09.2026 · **Source:** ביקורת-קוד עצמאית על Stage 1 (סעיף "מוטציה או עותק"), נרשם לפי בקשת דוד — לא לתקן עכשיו.

`home-workout.service.ts:1491-1493` — כש-`generateSingleOption`/`targetDifficulty` מבוקש (Custom Builder / drawer fast-path) ורק תוצאה אחת נוצרה: `results.push(results[0]); results.push(results[0]);`. זה לא שכפול-תוכן — זה **דחיפת הרפרנס הקיים פעמיים**. בפועל: `trio.options[0] === trio.options[1] === trio.options[2]` (זהות-אובייקט, לא רק שוויון-ערכים). זה המסלול החי של הקרוסלה בהום (`full-strength.generator.ts` וכו' קוראים ל-`generateHomeWorkoutTrio` בדיוק בתנאים האלה).

**המשמעות:** כל כתיבה ל-`opt.result.workout` של אופציה אחת כותבת בפועל לשלושתן, כי אין "שלוש" — יש אחת, עם שלוש דרכי-גישה. `stampResolvedLocation` (Stage 1, `home-workout.service.ts`) כבר עושה את זה היום — לולאה על `trio.options` שכותבת `executionLocation` לכל איבר — אבל **אידמפוטנטי ולא-מזיק כרגע**, כי היא כותבת את אותו ערך `location` שלוש פעמים לאותו אובייקט. שום דרישה-לוגית לא מפרה כתוצאה מזה, היום.

**מה שיישבר:** מגירת עריכת-האימון המתוכננת (ראה שיחת-הורה) — שבה המשתמש עורך אופציה *אחת* מהשלוש בנפרד — תתנגש בזה בשקט, בלי exception ובלי שגיאת-קומפילציה: עריכה על "אופציה 1" תופיע גם ב-2 וב-3, כי זה פיזית האותו אובייקט. אין בדיקת-זהות (`===`) קיימת היום שתתפוס את זה מוקדם.

**לפני שבונים עריכה פר-אופציה על גבי המסלול הזה — חייבים להפריד את שלושת האובייקטים** (למשל: שלושה clone עצמאיים במקום `results.push(results[0])` פעמיים, או guard מפורש שמזהה "single-option padded" ומטפל בו כמקרה-קצה נפרד בעריכה). **לא תוקן כאן — Stage 1 הוא תיקון-פלט בלבד, זה שינוי-מבנה-נתונים רחב יותר. דוד מחליט תזמון.**

---

## מפת-תוכן שכבת-המדיה — 5 ממצאים מהמדידה המתוקנת, 16.09.2026

**Opened:** 16.09.2026 · **Source:** המשך-ישיר של מדידת-כיסוי-המיקומים (execution-method-identity), אחרי שדוד תיקן את הסיווג מ"באג" ל"רשימת-צילומים". אלה חמישה ממצאים-לוואי מאותה סדרת-מדידה — לא לתקן, לרשום.

**1. המגירה והראנר לא מסכימים על הסבר חלופי.** `MasterExerciseView.tsx:623-636` — נעול-לשיטה במפורש ("method-LOCKED... Never another method's media"), לא מציג הסבר-של-שיטה-אחרת בשום מצב. ה-CTA "צפה בהסבר המלא" בראנר (`ExerciseVideoPlayer.tsx:33`) כן מציג, דרך ה-`.reduce` חוצה-השיטות ב-`media-resolution.utils.ts` (`fullTutorial`, היה `:138-141` לפני `commit 1c93cfac` על ענף נפרד — התיקון שם קבע park-first במפורש). שתי התנהגויות מכוונות, מתועדות בקוד, וסותרות זו את זו. ממתין להכרעת דוד — איזו משתי ההתנהגויות נכונה, או שהפער בכוונה.

**⚠️ תיקון-רשומה (16.09.2026, ביקורת):** הניסוח המקורי כאן ("התיקון שם... לא שינה את זה שהראנר נופל") רמז שה-park-first ב-`commit 1c93cfac` הוא no-op התנהגותי היום. **לא מדויק.** הביקורת מדדה בפועל: **42 מתוך 349 תרגילים עם 2+ שיטות (כ-12%) מקבלים URL וידאו שונה** תחת park-first לעומת סדר-המערך-הישן, ו-**125 תרגילים קיבלו עד כה וידאו של שיטה שאינה park** — מתוך ~341-346 נפילות-ישנות-חוצות-שיטות שנמדדו (המכנה המדויק תלוי במתודולוגיית הספירה ולא שוחזר עצמאית; ה-125 עצמו כן שוחזר, פעמיים, בשתי מתודולוגיות שונות), סתירה ישירה לטענה "559/559 נוחתים על park" שהופיעה בהערת-הקוד המקורית — תוקנה שם גם כן. המושפעים בפועל: משתמשים במיקומים בלי תוכן-עצמי (office/street/service) — קיבלו קודם וידאו-בית (המקרי-ראשון-במערך), מקבלים מעכשיו park. ההשפעה בפרודקשן **היום** מזערית כי PARK FORCE מנתב הכל ל-park ממילא — היא נכנסת לתוקף במלואה ברגע שמיקומים אחרים יתחילו לעבוד עצמאית (ר' ADR-004). הלוגיקה עצמה (park-first) **אושרה ע"י דוד ונשארת** — רק התיאור-הכתוב תוקן.

**2. service דולף דרך Priority 2.5/3 — `baselineGear('service') = ['bodyweight']` בלבד** (ראה גם הרשומה הקודמת על הדליפה, אם קיימת בענף אחר). מצב-צבא כפי-שהוא-היום יגיש בפועל תרגילי בית ופארק תחת תווית "צבא" — לא דחוי, לא מסונן, פשוט מוגש. כדי ש-service יהיה בעל משמעות אמיתית (לא רק תווית), הוא זקוק לשער-נוקשה כמו `park` (`method-selection.utils.ts:151`, "PARK: Strict location enforcement") — לא קיים לו כזה היום.

**3. פער-מיגרציה בין המיקומים — כל השוואת-כיסוי בין בית לפארק מערבבת שני דורות-שדות.** פארק: `legacy_only=268` מול `he_only` (Phase-5 `previewVideo`) `=25` — רוב תוכן-הפארק עדיין בשדה הישן. בית: ההפך המדויק — `he_only=143` מול `legacy_only=2` — כמעט כולו הוגר. **כל מספר-כיסוי שמשווה בית↔פארק בעתיד חייב לציין באיזה שדה נמדד** — אחרת ההשוואה משווה לא-רק-תוכן אלא גם דור-סכימה.
...
## שיוך שכונה אוטומטי — חסום עד שיהיו גבולות שכונה (2,443 רשומות, אפס גיאומטריה) — 23.09.2026

**Opened:** 23.09.2026 · **Source:** ביקורת זרימת הוספת-פארק (פיילוט שדרות), אחרי חיווט `resolveAuthorityForPoint` ל-authorityId. נרשם לפי בקשת דוד — לא לרדוף, בעיית דאטה לא קוד.

`park.neighborhoodId` נקבע היום **אך ורק ידנית** — dropdown בחירה ב-`ParkForm.tsx` (האדמין בוחר בעצמו), פלוס הגירת-reconciliation חד-פעמית מ-15.07.2026 שלא רצה יותר. 373 מתוך 1159 פארקים (32%) נושאים neighborhoodId — כולו ידני/היסטורי, אפס אוטומציה. אין שום קוד שמנחש שכונה מקואורדינטות.

**זה לא רק "עוד לא נבנה" — זה חסום על דאטה.** מתוך 2,755 רשומות `authorities`, **2,443 (89%) הן מסוג neighborhood/settlement** — ונבדק ישירות: **אף אחת מהן לא נושאת boundaryGeoJSON או radiusKm.** יש להן `coordinates` (נקודה בודדת בלבד), בדיוק כמו שדרות עצמה ברמת-העיר. גם כשגבולות-הערים ינחתו (הצ'אט המקביל שבונה את פייפליין מיפוי-העיר), זה **לא** יפתור שכונות — אלה שני datasets נפרדים לגמרי (עיר מול שכונה, אותה בעיה, רמה אחת עמוק יותר).

**אל תרדפו את זה.** רק כשגבולות/רדיוס יתחילו להיכתב על מסמכי neighborhood/settlement בפועל — יש טעם לחווט זיהוי-שכונה אוטומטי (אותו דפוס בדיוק כמו `resolveContributionAuthority`, רק עם רשימת השכונות של הרשות שכבר נפתרה, לא כל 2,755). כל resolver עתידי לרמת-שכונה חייב לסנן `type IN ['neighborhood','settlement']` בלבד, מאותה סיבה בדיוק כמו הסינון שנוסף ל-authorityId (PR #45, `feat/park-authority-auto-resolve`): `authorityId` לא יכול לקבל שכונה, ו-`neighborhoodId` לא צריך לקבל עיר.

---

## crossBrandFallback={false} במגירת הפארק יהפוך למשמעותי שוב כשתתווסף בחירת יצרן — 23.09.2026

**Opened:** 23.09.2026 · **Source:** תיקון תמונות-מתקנים בפיילוט שדרות (item 4, אותו צ'אט). לא לגעת עכשיו — בחירת יצרן מתקן דחויה בנפרד.

`EquipmentCard.tsx`'s `crossBrandFallback` prop (ברירת מחדל `true`) שולט אם מציגים תמונת-מותג-ראשון כש-`brandName=''`. הוסר מ-`ParkDetailSheet.tsx` (23.09.2026) כי כל תרומת-משתמש נכנסת עם `brandName=''` **תמיד** (האשף אף פעם לא שואל איזה יצרן) — כלומר ההגנה המקורית ("אל תציג מותג שגוי שגלוי-פיזית בפארק") תפסה בדיוק את הפארקים שבגללם היא נבדקה, ולא הגנה על כלום אמיתי.

**זה ישתנה ברגע שתתווסף בחירת יצרן מתקן לאשף** (אורבניקס/לודוס — item דחוי, ראה פרומפט המקור). ברגע שהמשתמש *כן* יכול לבחור יצרן אמיתי, `brandName=''` יחזור להיות איתות אמיתי ("המשתמש לא ידע/לא בחר" — לא "האשף אף פעם לא שאל") — ואז יש טעם אמיתי להחזיר את ההגנה, לפחות לחלק מהמקרים. שווה לחזור לשאלה הזו יחד עם עבודת בחירת-היצרן, לא לפני.
