...
## 🔵 עמידת ידיים — מודל תלת-ממדי (Handstand Triad) — תכנון מכוון, לא באג — 15.09.2026

**⚠️ אזהרה מפורשת לכל מי שיקרא: זו אינה אנומליה ואינה באג. אל תתקן, אל תאחד לרמה יחידה, ואל תסיר.**

**הכוונה של דוד (עתיד, לא ממומש):** לעמידת-ידיים (handstand/hspu) מודל שונה מכל שאר התוכניות — שלושה צירי-משנה, כוח/שיווי-משקל/גמישות, כל אחד 1-15 בנפרד, והרמה בתוכנית היא הממוצע שלהם. המטרה בהמשך: אימון מודולרי שנבנה מהחסרונות הספציפיים של המתאמן (למשל חזק אבל לא-יציב), ותיוג תרגילים באחוזים לפי כמה כל אחד דורש כוח/יציבות/גמישות — לא רק רמה שטוחה. בנוסף: העלאת כל תרגילי עמידת-הידיים לקטלוג, והוספת תרגילי גמישות-כתף ייעודיים (נחוצים לעמידת-ידיים תקינה, לא מכוסים כיום).

**סטטוס: עבודה מאחורי-הקלעים, לא הושלמה. קיימת כבר תצוגה חלקית בפאנל.**

### מה כבר קיים בקוד — עובדות מאומתות, לא ניחוש

**סכימת-הנתונים** — `exercise.types.ts:789-802` (`TargetProgramRef`): שלושה שדות אופציונליים על **רשומת ה-targetPrograms עצמה** (לא על שורש-התרגיל) — `strengthScore?: number`, `balanceScore?: number`, `mobilityScore?: number`. `level: number` נשאר שדה-חובה רגיל על אותה רשומה.

**⚠️ ממצא-דיוק — התיעוד הכתוב סותר את הקוד החי בשתי מקומות, אותה סתירה בדיוק:**
- `exercise.types.ts:793,797` — התגובה (doc-comment) אומרת **"(1–10)"** ו-**"clamped to [1, 10]"**.
- `ContentSection.tsx:156` — התגובה אומרת **"Clamped to [1, 10]"**.
- **הקוד בפועל, בשני המקומות שמממשים אותו, הוא 1-15**, לא 1-10: `ContentSection.tsx:162` — `Math.min(15, Math.max(1, raw))`; `ContentSection.tsx:166` — `Math.min(15, Math.max(1, Math.round((s + b + m) / 3)))`. גם ברירת-המחדל היא `?? 8` ("midpoint of 1–15", `ContentSection.tsx:149`, `TargetProgramSection.tsx` המקביל). **1-15 תואם בדיוק את מה שדוד כתב כאן ("כל אחד 1-15") — התגובות ב-1-10 הן שריד-תיעוד מיושן, לא הכוונה נוכחית.** לא תוקן — מסומן לתשומת-לב אם מישהו יגע בקוד הזה.

**UI מנהלתי — שתי מימושים נפרדים, לא אחד:** `TargetProgramSection.tsx` (שורות 98-247, כולל `handleTriadChange` בשורה 114) ו-`ContentSection.tsx` (שורות 127-356, כולל `handleTriadChange` נפרד בשורה 158) — **שני קומפוננטות שונות, כל אחת עם לוגיקה-כפולה מלאה** (חישוב-ברירת-מחדל, `handleTriadChange`, שלושה סליידרים, שורת-נוסחה תצוגתית). זוהות רק דרך שם-התוכנית (`_pName.includes('handstand') || _pName.includes('hspu') || ...includes('עמידת ידיים')`, `ContentSection.tsx:143-147`) — מוצג רק כשהתוכנית הנבחרת בעורך היא handstand/hspu.

**החישוב `level = round((strength+balance+mobility)/3)` כבר ממומש ופעיל, לא רק מתועד:** `handleTriadChange` (שני הקבצים) כותב את הציון-שהשתנה **וגם** את `level` המחושב-מחדש, לאותה רשומת-`targetPrograms`, באותה קריאה ל-`setTargetPrograms` (`ContentSection.tsx:166-174`). זה נשמר בפועל בעריכה-הבאה.

### מי צורך את שלושת הערכים היום — עובדה, לא המלצה

**`balanceScore` — כן, נצרך בפרודקשן. שני צרכנים בדיוק, שניהם קוראים ישירות מ-`targetPrograms`, לא מ-Level:**
1. `ContextualEngine.ts:222-246` — "Balance Score Gate": `Math.max(0, ...(exercise.targetPrograms ?? []).map(tp => (tp as any).balanceScore ?? 0))` (שורה 233-236). אם `balanceScore > 10` (סף קשיח, `BALANCE_GATE_THRESHOLD`, שורה 232 — **שים לב: הסף עצמו הוא 10, לא 15** — עקבי-פנימית עם ה-scale של 1-15, לא סתירה) — התרגיל נחסם מסשן-Push כללי, **אלא אם** handstand/hspu פעילים ב-`activeDomains` (שורה 238-239, `HANDSTAND_DOMAINS = new Set(['handstand','hspu'])`). מונע "דליפה" של תרגילי-שיווי-משקל-דומיננטיים (עמידות-ידיים חופשיות) לתוכנית-push רגילה.
2. `contextual-engine.types.ts:226` — שדה-דיווח (`excludedByBalanceGate`?) בטיפוס-התוצאה של הפילטר, לצורך observability — לא צריכת-לוגיקה נוספת, רק חשיפת-הספירה של השער מעל.
