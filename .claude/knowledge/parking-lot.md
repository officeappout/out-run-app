...
## ⚠️ `SKILL_TO_FOUNDATION_OFFSET = 9` — שודרג מ"מספר קסם" ל"סיכון בטיחות"
**Opened:** 08.09.2026 · **Source:** דוד, מלוג-האבחון — "מספר קסם +9 בלי מקור נראה לעין." **שודרג באותו יום**, אחרי ריצה שנייה: "תשדרג את פריט החניון הזה מ'מספר קסם' ל'סיכון בטיחות' — זה תרגילים שמתאמן עלול להיפצע בהם, לא רק אימון לא מדויק."

`onboarding-sync.service.ts:77` — `const SKILL_TO_FOUNDATION_OFFSET = 9;`, עם הערת-קוד (שורות 60-74) שכן מצטטת מקור: *"Root scale relationship (from master training spreadsheets)"*, תחת הכותרת `"David's Matrix"`. הנוסחה: `Foundational Track Level = Assessed Skill Level + 9`. מופעל רק ל-Path C (סקילים-בלבד) שלא עברו הערכת-push/pull נפרדת.

**לא "בלי מקור" — אלא מקור-מיוחס-לדוד-עצמו שלא ניתן לאימות מהקוד.** ההערה קוראת לזה "David's Matrix" ומפנה ל"גיליונות-אימון-ראשיים" — לא קובץ, לא לינק, שום דבר שניתן לפתוח ולבדוק מכאן.

**⚠️ שתי ריצות אמיתיות, שתי הוכחות שהקפיצה אינה תיאורטית:**
- ריצה קודמת (07.09.2026): `Skill→Foundation (multi-skill): push L14 (offset +9)` — כלומר סקיל אמיתי בסביבות L5 קפץ ל-L14.
- ריצה נוכחית (08.09.2026, לוג-האבחון): `Skill→Foundation (multi-skill): push L19 (offset +9)` — ומיד אחריו, בבחירת-התרגיל בפועל: `[1] שכיבות סמיכה בעמידת ידיים | programLevel=20 | Tier=hard`. **משתמש שנבחן בפלאנץ' L10 קיבל תרגיל L20, מתויג "קשה", בלי שום בדיקה בין הקפיצה של +9 לבין מה שהמתאמן באמת מסוגל אליו.**

זה לא עוד חוסר-דיוק בתצוגה — זו קפיצת-רמה אוטומטית, בלי גבול עליון נראה-לעין, שמזינה בחירת-תרגיל אמיתית לאימון אמיתי. אם זו באמת החלטה מכוונת של דוד ("David's Matrix") — עדיין ראוי לאשר שהיא נכונה גם בקצוות (L10→L19/L20), לא רק באמצע-הסולם. לא נחקר לעומק, לא תוקן.
...
לא נחקר לעומק, לא תוקן — נפרד מתיקון-ה-`oap` עצמו (שתיקן את שלב ה*-זיהוי*, לא את שלב ה*-בחירה*).
