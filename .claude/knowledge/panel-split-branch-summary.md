...
## 7. ⚠️ הבהרה חובה — 4 קומיטים נוספים על הענף הזה, לא חלק מהביקורת המבוקשת

**הענף `tmp-flag-flip` עבר `git rebase origin/main` (16.09.2026, נקי, אפס קונפליקטים, אומת ש-`git merge-base` תואם בדיוק את קצה `origin/main`) — hash-ים השתנו מהמצוטטים בהודעות-קודמות. Mapping (ישן→חדש):**

| ישן | חדש | מה |
|---|---|---|
| `690f5bdd` | `cca85ca0` | הפיצ'ר עצמו — **זה מה שמבוקש-לביקורת** |
| `98ef75ed` | `fe0fdc6d` | תיקון-תיעוד קטן (פיסטול) — קשור-ישירות לפיצ'ר, חלק מהביקורת |
| `8dc34ce6` | `d8a117a3` | המסמך הזה עצמו — כלי-לביקורת, לא תוכן-לביקורת |
| `49e23c78` | `2ade654b` | **לא קשור לפיצ'ר הזה בכלל** — ר' הסבר למטה |
| — | `2a24dcd5` | **לא קשור** — Handstand Triad |
| — | `eb0c0649` | **לא קשור** — כפילות-סנכרון בהרשמה |

**הסבר על `2ade654b` (היה `49e23c78`) — "B1 slug-map hits live recalculateMasterLevel too":** תיעוד-ממצא מ-`progression.service.ts` — למה `calisthenics_upper`'s רמת-מאסטר נתקעת לצמיתות למשתמש-סקילים אמיתי, שורש-בעיה שונה לגמרי מפיצול ה-NO_LEVEL בפאנל. **נכתב על הענף הזה רק כי זה היה הענף-הפעיל בזמן החקירה — לא בכוונה לצרף אותו לביקורת הפיצ'ר.**

**סיווג `2a24dcd5` ו-`eb0c0649` — אותה סיבה בדיוק, שני נושאי-חקירה נפרדים ולא-קשורים (Handstand Triad; כפילות-3-קריאות בהרשמת-סקילים).**

**המשמעות לביקורת: התייחסו לענף הזה כ-2 קומיטים לביקורת (`cca85ca0`+`fe0fdc6d`, עם `d8a117a3` כמסמך-מלווה) + 3 קומיטים-תיעוד עצמאיים שאין לקרוא/לבקר כחלק מה-diff של הפיצ'ר.** אם דוד מעדיף — ניתן להפריד את 3 המסמכים-הלא-קשורים לענף-`docs`-נפרד (לא מומש, ממתין להכרעה — ר' שאלת-הבהרה בצ'אט).
